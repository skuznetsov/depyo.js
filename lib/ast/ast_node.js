...
class ASTBinary extends ASTNode {
    ...
    codeFragment() {
        let result = new PycResult(`${this.left?.codeFragment() || "##ERROR##"}${this.op_str()}`);
...
class ASTStore extends ASTNode {
    ...
    codeFragment() {
        ...
        if (this.src instanceof ASTFunction) {
            ...
            if (this.src.op >= ASTBinary.BinOp.InplaceAdd && this.src.op != ASTBinary.BinOp.InvalidOp && this.dest.name == this.src.left.name) {
            ...
            switch (this.rettype) {
                ...
                case ASTReturn.RetType.YieldFrom:
                    if (this.value instanceof ASTAwaitable) {
                        result.add("await ");
                        result.lastLineAppend(this.expression.codeFragment());
                    } else {
...
class ASTSubscr extends ASTNode {
    ...
    codeFragment() {
        let result = new PycResult(this.name?.codeFragment() || '##ERROR##');
...
class ASTCondBlock extends ASTBlock {
    ...
    codeFragment() {
        ...
        if ([ASTBlock.BlockType.If, ASTBlock.BlockType.Elif, ASTBlock.BlockType.While].includes(this.blockType)) {
            ...
            result.lastLineAppend(" ", false);
            result.lastLineAppend(this.condition?.codeFragment() || "###ERROR###");
