...
class ASTNode {
    ...
    static renderList(list, openBracket, closeBracket, callback) {
        let result = new PycResult(openBracket);
        ...
        for (let node of list) {
            ...
            if (spacing == 0) {
                result.lastLineAppend((prevNode ? " " : "") + sourceFragment + ",")
            } else {
                if (spacing > 1) {
                    result.add("");
                }
                result.add((prevNode ? " " : "") + sourceFragment + ",")
            }
    ...
    codeFragment() {
        ...
        if (this.emptyBlock()) {
            ...
            for (let node of this.list) {
                let sourceFragment = node?.codeFragment() || "##ERROR##";
                let spacing = ASTNode.calculateSpacing(prevNode, node);
    
                if (prevNode && spacing == 0 && sourceFragment.length == 1) {
                    result.lastLineAppend((prevNode ? " " : "") + sourceFragment.toString() + ";")
                } else {
                    if (spacing > 1) {
                        result.add("");
                    }
...
class ASTChainStore extends ASTNodeList {
    ...
    codeFragment() {
        ...
        result.add(`${chain} = ${this.source.codeFragment()}`);
...
class ASTBinary extends ASTNode {
    ...
    codeFragment() {
        let result = `${this.left?.codeFragment() || "##ERROR##"}${this.op_str()}${this.right?.codeFragment() || "##ERROR##"}`;
...
class ASTCompare extends ASTBinary {
    ...
    codeFragment() {
        let result = `${this.left.codeFragment()}${this.op_str()}${this.right.codeFragment()}`;
...
class ASTSlice extends ASTBinary {
    ...
    constructor(parent, op, left, right) {
        left ||= new ASTNone(null, '');
        right ||= new ASTNone(null, '');
        super(parent, left, right, op);
    }
    ...
    codeFragment() {
        ...
        switch (this.op) {
            ...
            case ASTSlice.SliceOp.Slice3:
                str = `[${this.left.codeFragment()}:${this.right.codeFragment()}]`;
                break;
            default:
...
class ASTStore extends ASTNode {
    ...
    codeFragment() {
        ...
        if (this.src instanceof ASTFunction) {
            ...
            result.add(codeObject.SourceCode.codeFragment());
            ...
            if (classNode.bases.values.length > 0) {
                result.lastLineAppend(` (${classNode.bases.values.map(node => node?.codeFragment() || "#ERROR##").join(", ")})`);
            }
            result.lastLineAppend(":");
            ...
            result.add(`${this.dest?.codeFragment() || '##ERROR##'} = ${this.src?.codeFragment() || '##ERROR##'}`);
        }
            ...
            switch (this.rettype) {
                case ASTReturn.RetType.Return:
                    if (!this.value || this.value instanceof ASTNone) {
                        ...
                        result = `return ${this.value.codeFragment()}`;
                    }
                    break;
                case ASTReturn.RetType.Yield:
                    result = `yield ${this.value.codeFragment()}`;
                    break;
                case ASTReturn.RetType.YieldFrom:
                    if (this.value instanceof ASTAwaitable) {
                        result = `await ${this.expression.codeFragment()}`;
                    } else {
                        result = `yield from ${this.value.codeFragment()}`;
                    }
                    break;
            }
...
class ASTDelete extends ASTNode {
    ...
    codeFragment() {
        let result = `del ${this.value?.codeFragment() || '##ERROR##'}`;
...
class ASTFunction extends ASTNode {
    ...
    codeFragment() {
        ...
        result += argNames.join(", ") + ": ";
        codeObject.SourceCode.last.inLambda = true;
        result += codeObject.SourceCode.list.map(node => node.codeFragment()).join("; ") + ")";
        return result;
    }
...
class ASTCall extends ASTNode {
    ...
    codeFragment() {
        let result = `${this.func?.codeFragment() || "##ERROR##"}(`;
        let params = [];
        if (this.pparams?.length > 0) {
            params.push(this.pparams.map(node => node?.codeFragment() || "##ERROR##").join(', '));
        }
        if (this.kwparams?.length > 0) {
            params.push(this.kwparams.map(node => `${node?.key?.codeFragment().toString().replaceAll("'","")} = ${node?.value?.codeFragment() || "##ERROR##"}`).join(', '));
        }
        if (this.hasVar) {
...
class ASTImport extends ASTNode {
    ...
    codeFragment() {
        ...
        if (!this.stores.empty()) {
            ...
            result = `import ${this.name.codeFragment()}`;
            if (this.name.codeFragment() != this.alias?.codeFragment()) {
                result += ` as ${this.alias?.codeFragment()}`;
            }
        }
        return result;
    }
...
class ASTSubscr extends ASTNode {
    ...
    codeFragment() {
        let result = `${this.name?.codeFragment() || '##ERROR##'}[${this.key?.codeFragment() || "##ERROR##"}]`;
...
class ASTCondBlock extends ASTBlock {
    ...
    codeFragment() {
        ...
        if ([ASTBlock.BlockType.If, ASTBlock.BlockType.Elif, ASTBlock.BlockType.While].includes(this.blockType)) {
            result.lastLineAppend(`${this.negative ? " not" : ""} ${this.condition?.codeFragment() || "###ERROR###"}`);
        } else if ([ASTBlock.BlockType.Except].includes(this.blockType) && this.condition) {
            if (this.condition instanceof ASTStore) {
                result.lastLineAppend(` ${this.condition.src.codeFragment()} as ${this.condition.dest.codeFragment()}`);
            } else {
...
class ASTIterBlock extends ASTBlock {
    ...
    codeFragment() {
        ...
        result.add(`for ${this.index?.codeFragment() || "##ERROR##"} in ${this.iter?.codeFragment() || "##ERROR##"}:`);
        result.increaseIndent();
        this.nodes.map(node => result.add(node?.codeFragment() || "##ERROR##"));
        result.decreaseIndent();
...
class ASTWithBlock extends ASTBlock {
    ...
    codeFragment() {
        ...
        result.add(`with ${this.expr.codeFragment()}`);
        
        if (this.var) {
            result.lastLineAppend(` as ${this.var.codeFragment()}`);
        }
...
class ASTComprehension extends ASTNode {
    ...
    codeFragment() {
        let result = `[ ${this.result.codeFragment()} `;
