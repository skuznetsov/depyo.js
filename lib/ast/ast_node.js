...
class ASTNodeList extends ASTNode {
    ...
    codeFragment() {
        ...
        if (this.emptyBlock()) {
            ...
            for (let node of this.list) {
                ...
                if (this.requiresModuleSpacing(prevNode, node)) {
    ...
    requiresModuleSpacing(prevNode, node) {
        ...
        return isDefinition(prevNode) || prevNode instanceof ASTImport;
    }
    ...
    addDecorator(decorator) {
        ...
        if (this.src instanceof ASTFunction) {
            ...
            if (inLambda) {
                ...
                result.lastLineAppend(`def ${destName}(`);
            }
            ...
            let argIndex = 0;
                ...
                if (default_params.length === 1 && default_params[0] instanceof ASTTuple) {
                    ...
                    for (argIndex = 0; argIndex < codeObject.ArgCount; argIndex++) {
                        let argName = toVarName(codeObject.VarNames.Value?.[argIndex]);
...
class ASTAwaitable extends ASTNode {
    ...
    codeFragment() {
        // await <expression>
        return `await ${this.m_expr.codeFragment()}`;
    }
