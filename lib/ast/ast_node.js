...
class ASTStore extends ASTNode {
    ...
    codeFragment() {
        ...
        if (this.src instanceof ASTFunction) {
            ...
            if (codeObject.ArgCount) {
                ...
                if (codeObject.KWOnlyArgCount) {
                    ...
                    for (argIndex = 0; argIndex < codeObject.KWOnlyArgCount;) {
                        let argName = default_params[argIndex - (codeObject.KWOnlyArgCount - default_params.length)].name.codeFragment().replaceAll('"','');

                        if ((argIndex + 1) > (codeObject.KWOnlyArgCount - default_params.length)) {
                            argName += "=" + default_params[argIndex - (codeObject.KWOnlyArgCount - default_params.length)].value.codeFragment();
                        }
                        argIndex++;
                        argNames.push(argName);
                    }
        ...
        } else if (this.dest instanceof ASTName && this.dest.name == "__doc__") {
...
class ASTCondBlock extends ASTBlock {
    ...
    codeFragment() {
        ...
        if (this.blockType == ASTBlock.BlockType.If && this.negative && this.nodes.length == 1 && this.nodes[0] instanceof ASTRaise) {
...
class ASTComprehension extends ASTNode {
    ...
    codeFragment() {
        ...
        let result = `${openingBracket}${this.result.codeFragment()}`;
