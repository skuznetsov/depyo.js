...
function isTernaryExprNode(node) {
    ...
    // `raise` is a statement, not an expression — `if c: raise X` followed by a
    // value is a guard clause, never `X if c else value`. Collapsing it produces
    // invalid/wrong code like `raise ValueError() if not x else x` (3.12-3.14).
    if (node instanceof ASTRaise) return false;
    return true;
}
...
class ASTCondBlock extends ASTBlock {
    ...
    _codeFragmentImpl(result) {
        // This is the assert case — but ONLY when the single raised exception is
        // actually AssertionError. `assert X` compiles to `if not X: raise
        // AssertionError[(...)]` (via LOAD_ASSERTION_ERROR on 3.9+, LOAD_GLOBAL
        // AssertionError earlier), so the body must raise AssertionError to be an
        // assert. Without this guard `if not x: raise ValueError(msg)` collapses
        // into `assert x, msg`, silently changing the exception type (uuid.py).
        const _raisesAssertionError = (raiseNode) => {
            const exc = raiseNode?.params?.[0];
            if (exc instanceof ASTCall) {
                return exc.func instanceof ASTName && exc.func.name?.toString() === "AssertionError";
            }
            if (exc instanceof ASTName) {
                return exc.name?.toString() === "AssertionError";
            }
            return false;
        };
        if (this.blockType == ASTBlock.BlockType.If && this.negative && this.condition && this.nodes.length == 1 && this.nodes[0] instanceof ASTRaise && _raisesAssertionError(this.nodes[0])) {
        ...
        // NOTE: a `if cond: <value>` + `return <value>` -> `<value> if cond else
        // <ret>` reconstruction used to live here, but it was a pure false-positive
        // generator: it never produced a correct ternary for any fixture, while it
        // wrongly collapsed `if cond: foo()` (a discarded expression statement) plus
        // the function-tail `return x` into `foo() if cond else x` (which drops the
        // return). Real `return X if c else Y` reaches the STORE-context ternary
        // branch above or compiles to plain if/return, so removing this branch is a
        // zero-regression fix (verified: 0 fixture deltas with it disabled).
        } else {
            let header = this.type_str;
