        ...
        while (opOffset < code.length) {
            ...
            if (opCode.HasArgument) {
                ...
                if (opCode.HasConstant) {
                    ...
                    const reader = this.CodeObject.Reader;
                    const isNewLayout = reader?.versionCompare?.(3, 11) >= 0;
                    const varNames = this.CodeObject.VarNames?.Value ?? [];
                    const cellVars = this.CodeObject.CellVars?.Value ?? [];
                    const freeVars = this.CodeObject.FreeVars?.Value ?? [];

                    if (isNewLayout) {
                        // 3.11+: localsplusnames is constructed by CPython as the
                        // ordered slot list where each parameter/local has one slot
                        // even if it is also captured as a cell ("promoted" cells
                        // share the parameter slot). The deref opcodes index
                        // directly into this list:
                        //
                        //   localsplusnames = [
                        //     ...varNames,                         // slots 0..N-1
                        //     ...cellVars not already in varNames, // appended in cellvar order
                        //     ...freeVars,                         // last
                        //   ]
                        //
                        // The previous implementation assumed varNames and
                        // cellVars never overlap and subtracted varNames.length
                        // from the oparg, which misnames promoted cells and
                        // pushes legitimate cell-only entries off the end of the
                        // table. See depyo.py issue #5.
                        const inVar = new Set(varNames.map(v => v.toString()));
                        const cellOnly = [];
                        for (const cv of cellVars) {
                            if (!inVar.has(cv.toString())) cellOnly.push(cv);
                        }
                        const slot = opCode.Argument;
                        if (slot >= 0 && slot < varNames.length) {
                            opCode.FreeName = varNames[slot].toString();
                        } else if (slot >= varNames.length && slot < varNames.length + cellOnly.length) {
                            opCode.FreeName = cellOnly[slot - varNames.length].toString();
                        } else if (slot >= varNames.length + cellOnly.length &&
                                   slot < varNames.length + cellOnly.length + freeVars.length) {
                            opCode.FreeName = freeVars[slot - varNames.length - cellOnly.length].toString();
                        } else {
                            opCode.FreeName = `##FREEVAR_${opCode.Argument}##`;
                        }
                    } else {
                        // Pre-3.11: deref oparg indexes the concatenated tuple
                        // [cellVars | freeVars]; varNames is a separate table
                        // addressed by LOAD_FAST / STORE_FAST.
                        const slot = opCode.Argument;
                        if (slot >= 0 && slot < cellVars.length) {
                            opCode.FreeName = cellVars[slot].toString();
                        } else if (slot >= cellVars.length && (slot - cellVars.length) < freeVars.length) {
                            opCode.FreeName = freeVars[slot - cellVars.length].toString();
                        } else {
                            opCode.FreeName = `##FREEVAR_${opCode.Argument}##`;
                        }
                    }
