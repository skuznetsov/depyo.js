...
class OpCodes
{
    ...
    GetOffsetByOpCode(opCodeID, fromOffset = -1, toOffset = -1) {
        ...
        let endPosition =  toOffset < 0 ? this.Instructions.length : this.GetIndexByOffset(toOffset);
        if (startPosition > endPosition) {
            let tempPosition = startPosition;
            startPosition = endPosition;
            endPosition = tempPosition;
        }
        for (let position = startPosition; position < this.Instructions.length; position++) {
            if (this.Instructions[position].OpCodeID == opCodeID)
    ...
    DOMINATORS = [OpCodes.SETUP_LOOP, OpCodes.SETUP_EXCEPT, OpCodes.SETUP_FINALLY, OpCodes.SETUP_WITH, OpCodes.FOR_ITER];
