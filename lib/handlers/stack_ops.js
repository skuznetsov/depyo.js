...
function handleDupTop() {
    // Chained comparison (`a < x < b`): the compiler emits DUP_TOP + ROT_THREE
    // to share the middle value across the two COMPARE_OPs. Handle it FIRST,
    // before the match-candidate logic below — on 3.10 the `LOAD x; DUP_TOP`
    // prefix otherwise registers as a match subject and the whole `if a<x<b:`
    // is misreconstructed as `match x: case _ if ...`. Treating DUP_TOP+ROT_THREE
    // as a no-op (skip the ROT and the first short-circuit jump) collapses the
    // chain into a nested ASTCompare via the normal compare path.
    if (this.code.Next?.OpCodeID == this.OpCodes.ROT_THREE && this.dataStack.top() != null) {
...
function handleSwapA() {
    ...
    if (depth == 2) {
        // Chained comparison on 3.11+ (`a < x < b`): value-share prefix is
        // `SWAP 2; COPY 2; COMPARE_OP; <bare POP_JUMP>` (the 3.11+ replacement for
        // ≤3.10's DUP_TOP+ROT_THREE). Treat it as a no-op (skip COPY 2 + arm
        // skipNextJump) so the chain folds into a nested ASTCompare. Only the
        // CONDITIONAL form ends the compare with a bare POP_JUMP; the VALUE form
        // (`y = a<x<b`) inserts `COPY 1` (+ optional TO_BOOL) before the jump and
        // is already handled by the bool-chain path — firing there drops an
        // operand, so require the post-compare op to NOT be COPY 1.
        const instrs = this.code.Instructions || [];
        const ci = this.code.CurrentInstructionIndex;
        const copyI = instrs[ci + 1];
        const cmpI = instrs[ci + 2];
        const compareOps = [this.OpCodes.COMPARE_OP_A, this.OpCodes.COMPARE_OP,
                            this.OpCodes.IS_OP_A, this.OpCodes.CONTAINS_OP_A]
                           .filter(v => v !== undefined);
        if (copyI && copyI.OpCodeID === this.OpCodes.COPY_A && copyI.Argument === 2 &&
            cmpI && compareOps.includes(cmpI.OpCodeID) && this.dataStack.top() != null) {
            let k = ci + 3;
            while (instrs[k] && instrs[k].OpCodeID === this.OpCodes.CACHE) k++;
            const afterCmp = instrs[k];
            const isValueSticky = afterCmp && afterCmp.OpCodeID === this.OpCodes.COPY_A &&
                                  afterCmp.Argument === 1;
            if (!isValueSticky) {
                this.skipNextJump = true;
                this.code.GoNext();   // land on COPY 2; main loop's GoNext lands on COMPARE_OP
                return;
            }
        }
