...
function handleReturnConstA() {
    ...
    // Close blocks that have ended before processing the return — mirror
    // handleReturnValue. RETURN_CONST (3.12+) lands at if-block boundaries just
    // like RETURN_VALUE; without this the trailing `return` of `if c: return a`
    // gets appended to the still-open if-block instead of the parent, merging
    // both returns into the body (3.12/3.13).
    while (this.curBlock.end > 0 &&
           this.curBlock.end <= this.code.Current.Offset &&
           this.curBlock.blockType != AST.ASTBlock.BlockType.Main &&
           this.blocks.length > 1) {
        ...
        if (global.g_cliArgs?.debug) {
            console.log(`[handleReturnConstA] Closing ended block ${this.curBlock.type_str}(${this.curBlock.start}-${this.curBlock.end}) at offset ${this.code.Current.Offset}`);
        }
    ...
    // Mirror handleReturnValue: a return inside an if/else closes that block so
    // a following statement (e.g. the else-path return) lands at the parent
    // level rather than nesting under the if.
    if (!this.currentCase && [AST.ASTBlock.BlockType.If, AST.ASTBlock.BlockType.Else].includes(this.curBlock.blockType)
        && (this.object.Reader.versionCompare(2, 6) >= 0)) {
        const prev = this.curBlock;
        this.blocks.pop();
