...
function handleReturnValue() {
    if (this.currentMatch) {
        const shouldStartCase = (!this.inMatchPattern) || (this.patternOps?.length || 0) > 0 || !this.currentCase;
        if (shouldStartCase) {
    ...
    // 3.12 exception-group prolog sometimes leaves synthetic None; drop return entirely if immediately followed by PUSH_EXC_INFO.
    let value = this.dataStack.pop();
    ...
    if (value instanceof AST.ASTNone && nextOp?.OpCodeID === this.OpCodes.PUSH_EXC_INFO) {
        value = this.dataStack.pop() || new AST.ASTNone();
    }
    if (value == null) {
