...
function reconstructPattern(patternOps) {
    ...
    let matchClassOp = patternOps.find(op => op.type === 'MATCH_CLASS');
    ...
    if (matchClassOp) {
        ...
        let attrOpsStart = patternOps.indexOf(matchClassOp) + 1;
        let attrOps = patternOps.slice(attrOpsStart);
...
function flushCurrentCaseBody() {
    ...
    const {guard, bodyNodes: filtered} = summarizeCaseGuard(bodyNodes);
    ...
    if (global.g_cliArgs?.debug) {
        console.log(`[MATCH] Case body recorded with ${bodyNodes.length} node(s)`);
    }
...
function handlePopTop() {
    ...
    const readyForWildcard = this.currentMatch && !this.currentCase && !this.inMatchPattern && (this.patternOps?.length || 0) === 0;
...
function summarizeCaseGuard(nodes) {
    ...
    while (idx < nodes.length) {
        ...
        if (node instanceof AST.ASTCondBlock && node.blockType == AST.ASTBlock.BlockType.If) {
            const cond = node.condition;
            if (cond instanceof AST.ASTName || cond instanceof AST.ASTCompare || cond instanceof AST.ASTNamedExpr || cond instanceof AST.ASTBinary) {
                guard = cond;
                guardEndIndex = idx + 1;
                idx++;
                continue;
    ...
    let bodyNodes = nodes.slice(guardEndIndex);

    if (guard == null && bodyNodes.length >= 2) {
        let maybeReturn = bodyNodes[bodyNodes.length - 1];
        let maybeGuardExpr = bodyNodes[bodyNodes.length - 2];
        if (maybeReturn instanceof AST.ASTReturn && (maybeGuardExpr instanceof AST.ASTName || maybeGuardExpr instanceof AST.ASTCompare || maybeGuardExpr instanceof AST.ASTNamedExpr || maybeGuardExpr instanceof AST.ASTBinary)) {
            guard = maybeGuardExpr;
            bodyNodes = bodyNodes.slice(0, bodyNodes.length - 2).concat([maybeReturn]);
        }
    }

    return {guard, bodyNodes};
}
