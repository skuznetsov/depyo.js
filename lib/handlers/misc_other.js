...
function flushCurrentCaseBody() {
    ...
    while (bodyNodes.length > 0) {
        let tail = bodyNodes[bodyNodes.length - 1];
        if (tail instanceof AST.ASTName || tail instanceof AST.ASTTuple) {
            bodyNodes.pop();
            continue;
        }
        break;
    ...
    if (global.g_cliArgs?.debug) {
        const debugNodes = bodyNodes.map(n => `${n?.constructor?.name || typeof n}:${n?.codeFragment ? n.codeFragment() : ''}`);
