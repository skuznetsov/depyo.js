...
function handlePopTop() {
    ...
    if (this.inMatchPattern) {
        ...
        // Success path: POP_TOP comes after successful pattern checks, NOT after POP_JUMP
        let prevIsJump = [
        ...
        if (!prevIsJump) {
            ...
            // Create placeholder pattern (will improve in future commits)
            let placeholderPattern = new AST.ASTPattern(
                AST.ASTPattern.PatternType.Wildcard,
                '_'
            );

            // Create current case with placeholder
            this.currentCase = new AST.ASTCase(placeholderPattern, null);
        ...
        // If prevIsJump, this is failure path - just continue normal processing
    }
...
function handleReturnConstA() {
    ...
    // Match/case: Check if we're completing a case
    if (this.currentCase) {
        // Collect all nodes from curBlock as case body
        // (They were added during case body execution)
        let bodyNodes = [...this.curBlock.nodes];
        bodyNodes.push(node);  // Add the return
        ...
        if (global.g_cliArgs?.debug) {
            console.log(`[RETURN_CONST] Completed case with ${bodyNodes.length} nodes`);
            console.log(`  Total cases in match: ${this.currentMatch.cases.length}`);
        }

        // Clear curBlock nodes (they're now in case)
        this.curBlock.nodes = [];

        // Reset case
        this.currentCase = null;

        // Check if match is complete (no more cases coming)
        // Next case could start with:
        // 1. MATCH_SEQUENCE directly (same stack copy reused)
        // 2. POP_TOP + COPY + MATCH_SEQUENCE (new copy created)
        // 3. POP_TOP + MATCH_SEQUENCE (stack copy reused, common for wildcard cases)
        // Otherwise, match is complete
        let nextOpCodeId = this.code.Next?.OpCodeID;
        let nextNextOpCodeId = this.code.Next?.Next?.OpCodeID;
        let isMatchSequence = nextOpCodeId == this.OpCodes.MATCH_SEQUENCE;
        ...
        // If next is MATCH_SEQUENCE, or POP_TOP followed by (COPY or MATCH_SEQUENCE), another case is coming
        if (!isMatchSequence && !(nextIsPop && (nextNextIsCopy || nextNextIsMatch))) {
            // Match statement is complete!
            if (global.g_cliArgs?.debug) {
                console.log(`[RETURN_CONST] Match complete, adding to curBlock`);
            }

            // Pop subject from stack (it's been sitting there entire time)
            if (this.dataStack.length > 0 && this.dataStack.top() === this.matchSubject) {
                this.dataStack.pop();
            }

            // Add completed match to the block it started in (not curBlock, which may be nested)
            let targetBlock = this.matchParentBlock || this.curBlock;
            if (global.g_cliArgs?.debug) {
                console.log(`  Appending to ${targetBlock?.type_str}(${targetBlock?.start}-${targetBlock?.end})`);
            }
        ...
        // Don't add return to curBlock (it's in case body)
        return;
    }
