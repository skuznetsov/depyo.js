...
// Compound-condition variant of the duplicated-condition while (3.10-3.13):
//
//     <C1>; GUARD1        # top header: N groups, each = value ops + cond jump
//     <C2>; GUARD2        #   AND guards jump to exit; OR guards jump to body
//   body:
//     <B>
//     <C1 copy>; GUARD1'  # bottom header: a copy that re-tests the condition
//     <C2 copy>; GUARD2'  #   ending in a loop-back to body start
//   exit:
//
// The single-expression detector below only reads the FIRST group (condCount=1),
// so it mis-sizes the body and the remaining guards fold with the wrong operator.
// Here we detect the multi-group header explicitly: find the real body start (the
// loop-back target), the exit (max forward guard target), and the bottom-copy
// start (first instruction after the last body side effect from which the tail is
// side-effect-free). The While is opened over [bodyStart, bottomStart) with
// _guardExitRaw set so the merge path (processJumpOps) folds each subsequent guard
// by jump-target geometry into the correct `and`/`or` chain; [bottomStart, exit)
// is the dead copy.
function tryStartCompoundDupWhile(exitHint) {
    ...
    if (isInsideLoop(this.blocks)) return false;
    ...
    const backwardOps = new Set([OpCodes.JUMP_BACKWARD_A, OpCodes.JUMP_BACKWARD_NO_INTERRUPT_A,
        OpCodes.POP_JUMP_BACKWARD_IF_TRUE_A, OpCodes.POP_JUMP_BACKWARD_IF_FALSE_A].filter(v => v !== undefined));
    const valueOps = new Set([
        ...
        OpCodes.LOAD_FAST_BORROW_A, OpCodes.LOAD_FAST_BORROW_LOAD_FAST_BORROW_A, OpCodes.LOAD_SMALL_INT_A,
        ...
        OpCodes.CALL_METHOD_A, OpCodes.KW_NAMES_A, OpCodes.PUSH_NULL, OpCodes.LOAD_METHOD
    ].filter(v => v !== undefined));
    // A statement/block op means we've left the (side-effect-free) condition and
    // entered the body — used both to end the top header and to bound the bottom
    // copy. The bottom re-check contains only value ops + jumps, never these.
    const sideEffectOps = new Set([
        OpCodes.STORE_FAST_A, OpCodes.STORE_NAME_A, OpCodes.STORE_GLOBAL_A, OpCodes.STORE_DEREF_A,
        OpCodes.STORE_ATTR_A, OpCodes.STORE_ATTR, OpCodes.STORE_SUBSCR, OpCodes.STORE_SUBSCR_A,
        OpCodes.DELETE_FAST_A, OpCodes.DELETE_NAME_A, OpCodes.DELETE_GLOBAL_A, OpCodes.DELETE_DEREF_A,
        OpCodes.DELETE_SUBSCR, OpCodes.POP_TOP, OpCodes.POP_TOP_A, OpCodes.RETURN_VALUE,
        OpCodes.RETURN_VALUE_A, OpCodes.RETURN_CONST_A, OpCodes.RAISE_VARARGS_A, OpCodes.YIELD_VALUE,
        OpCodes.YIELD_VALUE_A, OpCodes.IMPORT_NAME_A, OpCodes.SETUP_FINALLY_A, OpCodes.SETUP_WITH_A,
        OpCodes.SETUP_ASYNC_WITH_A, OpCodes.POP_BLOCK, OpCodes.LIST_APPEND_A, OpCodes.SET_ADD_A,
        OpCodes.MAP_ADD_A, OpCodes.FOR_ITER_A, OpCodes.GET_ITER, OpCodes.STORE_FAST_STORE_FAST_A
    ].filter(v => v !== undefined));

    const nextRealIdx = (from) => {
        let j = from;
        while (j < instrs.length && instrs[j] && isNoise(instrs[j].OpCodeID)) j++;
        return (j < instrs.length && instrs[j]) ? j : -1;
    };
    const nextRealOffset = (from) => { const j = nextRealIdx(from); return j >= 0 ? instrs[j].Offset : null; };
    // Resolve a jump target for both forward guards and backward loop-backs.
    const resolveAny = (ins, idx) => {
        let arg = (ins.Argument || 0) * 2; // BPO-27129 (3.10+)
        const op = ins.OpCodeID;
        if (op === OpCodes.JUMP_ABSOLUTE_A) return arg;
        if (reader.versionCompare(3, 11) < 0 &&
            (op === OpCodes.POP_JUMP_IF_FALSE_A || op === OpCodes.POP_JUMP_IF_TRUE_A)) return arg; // 3.10 absolute
        const base = nextRealOffset(idx + 1);
        if (base === null) return -1;
        return backwardOps.has(op) ? base - arg : base + arg;
    };

    // Collect the top header groups: (value ops, cond jump) repeated. The header
    // ends when the run after a cond jump is body code (does not terminate in
    // another cond jump).
    const jumps = [];
    let i = guardIdx;
    while (jumps.length < 8) {
        const ins = instrs[i];
        if (!ins || !condJumps.has(ins.OpCodeID)) break;
        jumps.push({ idx: i, target: resolveAny(ins, i), op: ins.OpCodeID });
        const bodyCandidateIdx = nextRealIdx(i + 1);
        if (bodyCandidateIdx < 0) break;
        let k = bodyCandidateIdx;
        while (k < instrs.length && instrs[k] &&
               (valueOps.has(instrs[k].OpCodeID) || isNoise(instrs[k].OpCodeID))) k++;
        if (k < instrs.length && instrs[k] && condJumps.has(instrs[k].OpCodeID) && k > bodyCandidateIdx) {
            i = k; // another header group
            continue;
        }
        break; // run leads into the body
    }
    if (jumps.length < 2) return false;

    const lastGuardIdx = jumps[jumps.length - 1].idx;
    const bodyStartIdx = nextRealIdx(lastGuardIdx + 1);
    if (bodyStartIdx < 0) return false;
    const bodyStart = instrs[bodyStartIdx].Offset;

    // Exit = the farthest-forward guard target (AND: all guards; OR: the final
    // fall-through guard). Every guard must target the body start or the exit.
    let exit = 0;
    for (const jm of jumps) {
        if (jm.target === bodyStart) continue;
        if (jm.target > bodyStart) { exit = Math.max(exit, jm.target); continue; }
        return false; // guard into body interior — not a while header
    }
    if (!exit || exit <= bodyStart) return false;

    // Validate: at least one loop-back jump inside (bodyStart, exit) returns to
    // the body start. Its presence distinguishes a loop from a plain if.
    let hasLoopBack = false;
    for (let j = bodyStartIdx; j < instrs.length; j++) {
        const ins = instrs[j];
        if (!ins || ins.Offset >= exit) break;
        if (!condJumps.has(ins.OpCodeID) && !backwardOps.has(ins.OpCodeID) &&
            ins.OpCodeID !== OpCodes.JUMP_ABSOLUTE_A) continue;
        if (resolveAny(ins, j) === bodyStart) { hasLoopBack = true; break; }
    }
    if (!hasLoopBack) return false;

    // Bottom copy start: the first instruction after the last body side effect,
    // from which [bottomStart, exit) is entirely side-effect-free (only the
    // condition re-evaluation and its loop-back jumps).
    let lastSideEffectIdx = -1;
    for (let j = bodyStartIdx; j < instrs.length; j++) {
        const ins = instrs[j];
        if (!ins || ins.Offset >= exit) break;
        if (sideEffectOps.has(ins.OpCodeID)) lastSideEffectIdx = j;
    }
    if (lastSideEffectIdx < 0) return false; // no body side effect → not this shape
    const bottomStartIdx = nextRealIdx(lastSideEffectIdx + 1);
    if (bottomStartIdx < 0 || instrs[bottomStartIdx].Offset >= exit) return false;
    const bottomStart = instrs[bottomStartIdx].Offset;
    if (!(bottomStart > bodyStart && bottomStart < exit)) return false;
    // The bottom copy must begin the condition re-eval with a value op (the first
    // opcode of C1'), never a jump — a jump here would mean we mis-split the body.
    if (!valueOps.has(instrs[bottomStartIdx].OpCodeID)) return false;

    const whileBlk = new AST.ASTCondBlock(AST.ASTBlock.BlockType.While, bodyStart, bottomStart, null, false);
    whileBlk.line = this.code.Current.LineNo;
    whileBlk._guardExitRaw = exit;
    this.blocks.push(whileBlk);
    this.curBlock = whileBlk;
    (this.chainDeadRegions ||= []).push({ from: bottomStart, to: exit });
    return true;
}
...
// Chained-comparison variant of the duplicated-condition while (3.10-3.13):
//
//     <a>; <x>; DUP/ROT (3.10) / SWAP+COPY (3.11+)
//     COMPARE; GUARD -> cleanup     # middle guard(s), consumed by the chain fold
//     COMPARE; GUARD -> exit        # final guard (this instruction), cond = a<x<b
//     JUMP_FORWARD body             # all-true path over the POP_TOP cleanup
//     POP_TOP; [tail copy]          # cleanup + duplicated block tail
//   body:
//     <B>
//     <chain copy>; loop-back body  # verbatim re-test ending in a jump to body
//     POP_TOP; [tail copy]
//   exit:
//
// Neither dup-while detector above fires: the body starts at a JUMP_FORWARD
// target (not right after the guard), the middle guards target the POP_TOP
// cleanup (neither body nor exit), and the loop-back targets the body, never
// the guard. Without this the loop renders as a single-iteration `if a<x<b:`.
// Detection is gated on the folded nested-ASTCompare condition AND on the
// bottom re-test being an opcode-for-opcode copy of the top chain, so a
// `while True:` opening an `if a<x<b:` body (whose loop-back also targets the
// body start) can never match.
function tryStartChainedCompareDupWhile(exitRaw, cond) {
    const OpCodes = this.OpCodes;
    const reader = this.object?.Reader;
    if (!reader || reader.versionCompare(3, 10) < 0 || reader.versionCompare(3, 14) >= 0) return false;
    if (isInsideLoop(this.blocks)) return false;
    if (!(cond instanceof AST.ASTCompare) || !(cond.left instanceof AST.ASTCompare)) return false;

    const instrs = this.code.Instructions || [];
    const guardIdx = this.code.CurrentInstructionIndex;
    const cur = instrs[guardIdx];
    if (!cur || ![OpCodes.POP_JUMP_IF_FALSE_A, OpCodes.POP_JUMP_FORWARD_IF_FALSE_A,
                  OpCodes.INSTRUMENTED_POP_JUMP_IF_FALSE_A].filter(v => v !== undefined)
                 .includes(cur.OpCodeID)) return false;
    if (!(exitRaw > cur.Offset)) return false;
    ...
    const valueOps = new Set([
        ...
        OpCodes.LOAD_ATTR_A, OpCodes.LOAD_DEREF_A, OpCodes.LOAD_CLASSDEREF_A, OpCodes.LOAD_METHOD_A,
        OpCodes.LOAD_FAST_CHECK_A, OpCodes.LOAD_FAST_AND_CLEAR_A, OpCodes.LOAD_FAST_LOAD_FAST_A,
        OpCodes.LOAD_SMALL_INT_A, OpCodes.COMPARE_OP_A, OpCodes.COMPARE_OP, OpCodes.IS_OP_A,
        OpCodes.CONTAINS_OP_A, OpCodes.BINARY_OP_A, OpCodes.BINARY_OP, OpCodes.BINARY_SUBSCR,
        OpCodes.BINARY_SUBSCR_A, OpCodes.UNARY_NOT, OpCodes.UNARY_NEGATIVE, OpCodes.UNARY_POSITIVE,
        OpCodes.UNARY_INVERT, OpCodes.TO_BOOL, OpCodes.DUP_TOP, OpCodes.DUP_TOP_A, OpCodes.ROT_TWO,
        OpCodes.ROT_THREE, OpCodes.SWAP_A, OpCodes.COPY_A, OpCodes.LOAD_ATTR, OpCodes.CALL_A,
        OpCodes.CALL_FUNCTION_A, OpCodes.CALL_METHOD_A, OpCodes.PUSH_NULL, OpCodes.LOAD_METHOD,
        OpCodes.BINARY_ADD, OpCodes.BINARY_SUBTRACT, OpCodes.BINARY_MULTIPLY, OpCodes.BINARY_MODULO
    ].filter(v => v !== undefined));
    ...
    // The all-true path: a JUMP_FORWARD right after the final guard, over the
    // POP_TOP cleanup, into the loop body.
    const jfIdx = nextRealIdx(guardIdx + 1);
    if (jfIdx < 0 || instrs[jfIdx].OpCodeID !== OpCodes.JUMP_FORWARD_A) return false;
    const jf = instrs[jfIdx];
    let jfArg = (jf.Argument || 0) * 2; // BPO-27129 (3.10+)
    const bodyStart = jf.Offset + (jf.Size || 2) + jfArg;
    if (!(bodyStart > jf.Offset && bodyStart < exitRaw)) return false;

    // Walk back over a condition run (value ops + chain guards), collecting the
    // non-noise opcodes in order. Capped: chains are short.
    const walkBackRun = (fromIdx, floorIdx) => {
        const ops = [];
        let i = fromIdx;
        while (i > floorIdx && ops.length < 64) {
            const op = instrs[i]?.OpCodeID;
            if (op === undefined) break;
            if (isNoise(op)) { i--; continue; }
            if (!valueOps.has(op) && !condJumps.has(op)) break;
            ops.unshift(op);
            i--;
        }
        return { ops, startIdx: i + 1 };
    };
    const top = walkBackRun(guardIdx - 1, -1);
    // Chain signature: the value-share prefix must be present in the top run.
    const hasShare = (top.ops.includes(OpCodes.DUP_TOP) && top.ops.includes(OpCodes.ROT_THREE)) ||
                     (top.ops.includes(OpCodes.SWAP_A) && top.ops.includes(OpCodes.COPY_A));
    if (!hasShare) return false;

    // Find the loop-back: a jump inside (bodyStart, exit) targeting the body
    // start. 3.10: POP_JUMP_IF_TRUE (absolute); 3.11: POP_JUMP_BACKWARD_IF_*;
    // 3.12/3.13: JUMP_BACKWARD after the chain's exit guard.
    const backwardOps = new Set([OpCodes.JUMP_BACKWARD_A, OpCodes.JUMP_BACKWARD_NO_INTERRUPT_A,
        OpCodes.POP_JUMP_BACKWARD_IF_TRUE_A, OpCodes.POP_JUMP_BACKWARD_IF_FALSE_A].filter(v => v !== undefined));
    const resolveAny = (ins, idx) => {
        let arg = (ins.Argument || 0) * 2;
        const op = ins.OpCodeID;
        if (op === OpCodes.JUMP_ABSOLUTE_A) return arg;
        if (reader.versionCompare(3, 11) < 0 &&
            (op === OpCodes.POP_JUMP_IF_FALSE_A || op === OpCodes.POP_JUMP_IF_TRUE_A)) return arg; // 3.10 absolute
        const j = nextRealIdx(idx + 1);
        if (j < 0) return -1;
        return backwardOps.has(op) ? instrs[j].Offset - arg : instrs[j].Offset + arg;
    };
    let loopBackIdx = -1;
    for (let i = jfIdx + 1; i < instrs.length; i++) {
        const ins = instrs[i];
        if (!ins || ins.Offset >= exitRaw) break;
        if (ins.Offset <= bodyStart) continue;
        if (!condJumps.has(ins.OpCodeID) && !backwardOps.has(ins.OpCodeID) &&
            ins.OpCodeID !== OpCodes.JUMP_ABSOLUTE_A) continue;
        if (resolveAny(ins, i) === bodyStart) { loopBackIdx = i; break; }
    }
    if (loopBackIdx < 0) return false;

    // The bottom re-test must be an opcode-for-opcode copy of the top chain's
    // tail (the top run may over-extend backwards into unrelated condition
    // code, so compare as a suffix). This is the semantic gate: anything that
    // is not a duplicated chain re-test mismatches and falls through.
    const bottom = walkBackRun(loopBackIdx - 1, jfIdx);
    let bottomOps = bottom.ops;
    // 3.12/3.13: the loop-back is an unconditional JUMP_BACKWARD and the
    // re-test's own exit guard sits right before it — the counterpart of the
    // current (top) guard, which the top run likewise excludes. Drop it so the
    // runs compare copy-to-copy. 3.10/3.11 loop back via the conditional
    // itself, so there is nothing to drop.
    const uncondBack = [OpCodes.JUMP_BACKWARD_A, OpCodes.JUMP_BACKWARD_NO_INTERRUPT_A,
                        OpCodes.JUMP_ABSOLUTE_A].includes(instrs[loopBackIdx].OpCodeID);
    if (uncondBack && bottomOps.length && condJumps.has(bottomOps[bottomOps.length - 1])) {
        bottomOps = bottomOps.slice(0, -1);
    }
    if (bottomOps.length < 4) return false;
    if (bottomOps.length > top.ops.length) return false;
    // The copy must itself contain the value-share prefix — a full chain
    // re-test, not just a tail that happens to end like one.
    const bottomHasShare = (bottomOps.includes(OpCodes.DUP_TOP) && bottomOps.includes(OpCodes.ROT_THREE)) ||
                           (bottomOps.includes(OpCodes.SWAP_A) && bottomOps.includes(OpCodes.COPY_A));
    if (!bottomHasShare) return false;
    const topTail = top.ops.slice(top.ops.length - bottomOps.length);
    for (let k = 0; k < bottomOps.length; k++) if (topTail[k] !== bottomOps[k]) return false;
    const bottomStart = instrs[bottom.startIdx].Offset;
    if (!(bottomStart > bodyStart && bottomStart < exitRaw)) return false;

    const whileBlk = new AST.ASTCondBlock(AST.ASTBlock.BlockType.While, bodyStart, bottomStart, null, false);
    whileBlk.line = this.code.Current.LineNo;
    this.blocks.push(whileBlk);
    this.curBlock = whileBlk;
    (this.chainDeadRegions ||= []).push({ from: bottomStart, to: exitRaw });
    return true;
}

// Scan a `while C1 or C2 ...:` loop header starting at the current conditional
// jump (the first header jump). A header is a run of groups, each = value ops +
// one conditional jump; the body starts right after the last group's jump.
// Every header jump must target either the body start (OR short-circuit into
// the body) or the loop exit (past the loop-back at `backJumpOffset`); a jump
// into the middle of the body (an ordinary `if` inside a loop) disqualifies the
// chain. Returns { bodyStart, exitRaw } or null. Used when the first header
// jump targets the body (OR), which the plain `offs > backJump.Offset` while
// test cannot see (it only looks at the first jump's own target).
function scanLoopHeaderChain(ctx, backJumpOffset) {
    ...
    const valueOps = new Set([
        ...
        OpCodes.BINARY_OP_A, OpCodes.BINARY_OP, OpCodes.BINARY_SUBSCR, OpCodes.BINARY_SUBSCR_A,
        OpCodes.UNARY_NOT, OpCodes.UNARY_NEGATIVE, OpCodes.UNARY_POSITIVE, OpCodes.UNARY_INVERT,
        OpCodes.TO_BOOL, OpCodes.LOAD_ATTR, OpCodes.CALL_A, OpCodes.CALL_FUNCTION_A,
        OpCodes.CALL_METHOD_A, OpCodes.PUSH_NULL, OpCodes.LOAD_METHOD,
        OpCodes.BINARY_ADD, OpCodes.BINARY_SUBTRACT, OpCodes.BINARY_MULTIPLY, OpCodes.BINARY_MODULO
    ].filter(v => v !== undefined));

    // Collect header groups greedily: (value-op run, cond jump)*.
    const jumps = []; // { idx, target, afterOffset } afterOffset = first real op after the jump
    let i = startIdx;
    const maxGroups = 8;
    while (jumps.length < maxGroups) {
        const ins = instrs[i];
        if (!ins || !condJumps.has(ins.OpCodeID)) break;
        const target = resolveJumpTarget(ins, reader, instrs[i + 1]?.Offset, instrs, i);
        // First real (non-noise) instruction after this jump.
        let j = i + 1;
        while (j < instrs.length && instrs[j] && isNoise(instrs[j].OpCodeID)) j++;
        if (j >= instrs.length || !instrs[j]) break;
        jumps.push({ idx: i, target, afterOffset: instrs[j].Offset });
        // Walk the next value-op run; if it ends at another cond jump, continue the chain.
        let k = j;
        while (k < instrs.length && instrs[k] &&
               (valueOps.has(instrs[k].OpCodeID) || isNoise(instrs[k].OpCodeID))) k++;
        if (k < instrs.length && instrs[k] && condJumps.has(instrs[k].OpCodeID) && k > j) {
            i = k;
            continue;
        }
        break;
    ...
    // Validate: body starts right after the last header jump; every header jump
    // targets the body start or the exit (past the loop-back).
    const bodyStart = jumps[jumps.length - 1].afterOffset;
    let exitRaw = 0;
    for (const jm of jumps) {
        if (jm.target === bodyStart) continue;
        if (jm.target > backJumpOffset) { exitRaw = Math.max(exitRaw, jm.target); continue; }
        return null; // jump into the body interior — not a loop header chain
    }
    if (!exitRaw) return null;
    return { bodyStart, exitRaw };
}
...
function processJumpOps() {
    ...
    if (!this.inMatchPattern &&
        ...
        const dupWhile = tryStartCompoundDupWhile.call(this, rawJumpTarget) ||
                         tryStartChainedCompareDupWhile.call(this, rawJumpTarget, cond) ||
                         tryStartDuplicatedConditionWhile.call(this, rawJumpTarget, offs);
        if (!dupWhile) {
            const backJump = findBackwardJump(this.code, this.code.CurrentInstructionIndex, this.code.Current.Offset, backJumpOpcodes);
            if (backJump) {
                let loopEnd = 0, exitRaw = 0;
                if (offs > backJump.Offset) {
                    // First header jump targets the loop exit (single condition, or
                    // the AND shape): body extends past the backward jump target.
                    loopEnd = offs;
                    exitRaw = rawJumpTarget;
                } else if (this.object.Reader.versionCompare(3, 8) >= 0) {
                    // First header jump targets the body (`while a or b:`): the
                    // exit is a LATER header jump's target. Chain-scan the header.
                    // 3.8+ only — older versions mark loops with SETUP_LOOP and
                    // are reconstructed by that machinery.
                    const hdr = scanLoopHeaderChain(this, backJump.Offset);
                    if (hdr && rawJumpTarget === hdr.bodyStart) {
                        [loopEnd] = this.code.FindEndOfBlock(hdr.exitRaw);
                        exitRaw = hdr.exitRaw;
                    }
                }
                if (loopEnd > backJump.Offset) {
                    const loopStart = backJump.JumpTarget;
                    const whileBlk = new AST.ASTCondBlock(AST.ASTBlock.BlockType.While, loopStart, loopEnd, null, false);
                    whileBlk.line = this.code.Current.LineNo;
                    // Target-geometry header folding is a 3.8+ shape (SETUP_LOOP-era
                    // loops fold through the legacy heuristic; see the merge path).
                    if (this.object.Reader.versionCompare(3, 8) >= 0) {
                        whileBlk._guardExitRaw = exitRaw;
                    }
                    this.blocks.push(whileBlk);
                    this.curBlock = whileBlk;
                }
    ...
    // Conditional `break` in an infinite loop: `while True: … if c: break`
    // compiles the guard as a single POP_JUMP_IF_*/exit with no separate
    // JUMP_FORWARD, so the jump target is past the loop's back-edge (the loop
    // exit). Without recognising it the jump renders as `if not c: pass` and the
    // loop never terminates. Scoped to infinite loops (condition == null): a
    // conditional `while COND:` reaches its exit through the header guard, not a
    // body break, so this never disturbs those.
    if (!this.inMatchPattern &&
        condJumpOpcodes.has(this.code.Current.OpCodeID) &&
        isInsideLoop(this.blocks) &&
        // Skip the loop's own entry guard: a top-tested `while COND:` reaches
        // this with the While still uninitialised (its condition is assigned
        // just below), and that guard must become the loop condition, not a
        // break. A real break runs inside an already-established infinite loop
        // (condition already fixed to True by maybeOpenInfiniteLoop → inited).
        (this.curBlock.blockType !== AST.ASTBlock.BlockType.While || this.curBlock.inited)) {
        let loop = null;
        ...
        if (loop && loop.blockType === AST.ASTBlock.BlockType.While &&
            loop.inited && loop.condition == null && loop.end > 0 &&
            this.code.Current.Offset < loop.end && rawJumpTarget > loop.end) {
            // `if <cond>: break`. The jump is taken (→ exit) when the guard fires,
            // so the branch keyword is break and the condition polarity is the
            // opposite of the fall-through `if` (IF_TRUE → `if cond`, IF_FALSE →
            // `if not cond`). The condition was already popped above. Open a
            // one-statement If holding the break.
            const brkIf = new AST.ASTCondBlock(AST.ASTBlock.BlockType.If,
                this.code.Current.Offset, this.code.Next?.Offset, cond, !neg);
            brkIf.line = this.code.Current.LineNo;
            if (popped) brkIf.init(popped);
            brkIf.append(new AST.ASTKeyword(AST.ASTKeyword.Word.Break));
            this.curBlock.append(brkIf);
            return;
        } else if (this.object.Reader.versionCompare(3, 8) >= 0 &&
                   this.object.Reader.versionCompare(3, 10) < 0 &&
                   loop && [AST.ASTBlock.BlockType.While, AST.ASTBlock.BlockType.For]
                       .includes(loop.blockType) &&
                   loop.inited &&
                   rawJumpTarget === loop.start &&
                   this.code.Next?.OpCodeID === this.OpCodes.JUMP_ABSOLUTE_A &&
                   (this.code.Next.Argument || 0) > this.code.Next.Offset) {
            // 3.8/3.9 bottom test: `if c: break` as the LAST loop-body
            // statement compiles to a backward conditional jump to the loop
            // top with the break's JUMP_ABSOLUTE→exit falling through. This
            // holds for `while True:` (opened by maybeOpenBottomTestedLoop)
            // and equally for top-tested `while COND:`/`for` loops (the
            // peephole redirects the if's false branch straight to the loop
            // top). Jump taken (→ top) continues the loop, fall-through
            // breaks, so the If keeps the raw polarity (IF_FALSE →
            // `if c: break`, IF_TRUE → `if not c: break`).
            const brkIf = new AST.ASTCondBlock(AST.ASTBlock.BlockType.If,
                this.code.Current.Offset, this.code.Next?.Offset, cond, neg);
            brkIf.line = this.code.Current.LineNo;
            if (popped) brkIf.init(popped);
            brkIf.append(new AST.ASTKeyword(AST.ASTKeyword.Word.Break));
            this.curBlock.append(brkIf);
            // The break's JUMP_ABSOLUTE (and the dead loop-back the peephole
            // leaves after it) are represented by the `break` keyword — skip.
            const F = this.code.Next;
            let deadTo = F.Offset + (F.Size || 2);
            const afterIdx = this.code.CurrentInstructionIndex + 2;
            const after = (this.code.Instructions || [])[afterIdx];
            if (after && after.OpCodeID === this.OpCodes.JUMP_ABSOLUTE_A &&
                (after.Argument || 0) === loop.start) {
                deadTo = after.Offset + (after.Size || 2);
            }
            this.addDeadRegion(F.Offset, deadTo);
            return;
        }
    }
    ...
    if (this.curBlock.size == 0 && !this.curBlock.inited
        ...
        top.condition = cond;
        top.negative = neg;
        // Remember this header jump's raw target so a following header jump can
        // classify the fold: previous jump to the exit => AND, to the body => OR.
        top._condJumpRawTarget = rawJumpTarget;
        if (popped) {
            top.init(popped);
        }
        ...
        if (top.blockType == AST.ASTBlock.BlockType.While
                && top._guardExitRaw !== undefined
                && top._condJumpRawTarget !== undefined
                && rawJumpTarget > this.code.Current.Offset) {
            // Target-geometry fold only applies when the current header jump goes
            // forward (to the body or the exit). A backward conditional here (seen
            // in corpus bytecode where the re-test jumps to the loop top) carries
            // none of that geometry — leave it to the legacy heuristic below.
            // While header fold with explicit jump-target geometry: the previous
            // header jump either short-circuits to the loop exit (AND: falsy lhs
            // skips the loop) or into the body (OR: truthy lhs enters the loop).
            // A polarity/target mismatch (`while not a or b:` / `while not a and
            // b:`) negates the lhs.
            let lhs = cond1;
            const prevToExit = top._condJumpRawTarget === top._guardExitRaw;
            const wantNegated = prevToExit ? top.negative : !top.negative;
            if (wantNegated) {
                lhs = new AST.ASTUnary(cond1, AST.ASTUnary.UnaryOp.Not);
                lhs.line = this.code.Current.LineNo;
            }
            newcond = new AST.ASTBinary(lhs, cond,
                prevToExit ? AST.ASTBinary.BinOp.LogicalAnd : AST.ASTBinary.BinOp.LogicalOr);
            newcond.line = this.code.Current.LineNo;
            // Preserve the loop geometry established at creation (body end may be
            // before the exit on 3.10-3.13 duplicated-condition loops).
            ifblk = new AST.ASTCondBlock(top.blockType, top.start, top.end, newcond, neg);
            ifblk.line = top.line;
            ifblk._guardExitRaw = top._guardExitRaw;
            ifblk._condJumpRawTarget = rawJumpTarget;
        } else {
...
function handleJumpAbsoluteA() {
    ...
    // 3.8-3.10 `continue`: an unconditional JUMP_ABSOLUTE back to the loop top
    // from inside a conditional in the loop body. Without this, the `curBlock.end
    // == Next.Offset` early-return below drops the jump and the `if` renders as
    // `if c: pass` (the trailing body then hoists out of the loop). The loop's own
    // loop-back arrives with curBlock == the loop itself (not an If/Elif/Else), so
    // the block-type guard keeps this from firing on it. Mirrors the 3.11+
    // handleJumpBackwardA continue detection: a dup-condition while's continue
    // targets the condition top (offs < body-start = loop.start); a top-tested /
    // for loop's continue targets the loop top (offs == loop.start) and a LATER
    // back-edge to the same target (the true loop-back) must exist.
    // Require an EMPTY conditional body: `if c: continue` reaches this jump with
    // the If still empty (the continue IS the body). A non-empty body means the
    // jump is instead the natural if/else-branch exit to the loop re-test (e.g.
    // `if c: X` / `else: Y`, both tails jumping to the loop top), which must stay
    // a plain branch, not gain a spurious `continue`.
    if (this.object.Reader.versionCompare(3, 8) >= 0 &&
        this.curBlock.size === 0 &&
        [AST.ASTBlock.BlockType.If, AST.ASTBlock.BlockType.Elif,
         AST.ASTBlock.BlockType.Else].includes(this.curBlock.blockType) &&
        !this.hasTerminatingKeyword(this.curBlock)) {
        let loop = null;
        for (let bi = this.blocks.length - 1; bi >= 0; bi--) {
            const b = this.blocks[bi];
            if ([AST.ASTBlock.BlockType.While, AST.ASTBlock.BlockType.For,
        ...
        if (loop) {
            let isContinue = offs < loop.start;
            if (!isContinue && offs === loop.start && typeof this.getBackJumpMap === 'function') {
                const farthest = this.getBackJumpMap().get(offs);
                isContinue = !!(farthest && farthest.offset > this.code.Current.Offset);
            }
            // `if c: pass` at a loop-body tail compiles identically to `if c:
            // continue` (empty branch, jump to loop top). Disambiguate by the
            // if's fall-through: if the instruction right after this block begins
            // another conditional test, this is an `if/elif` whose empty leading
            // branch is `pass`, not a `continue`. A real `continue` falls through
            // to plain loop body that it skips.
            if (isContinue) {
                const condTest = new Set([
                    this.OpCodes.POP_JUMP_IF_FALSE_A, this.OpCodes.POP_JUMP_IF_TRUE_A,
                    this.OpCodes.POP_JUMP_FORWARD_IF_FALSE_A, this.OpCodes.POP_JUMP_FORWARD_IF_TRUE_A,
                    this.OpCodes.POP_JUMP_IF_NONE_A, this.OpCodes.POP_JUMP_IF_NOT_NONE_A,
                    this.OpCodes.JUMP_IF_FALSE_A, this.OpCodes.JUMP_IF_TRUE_A,
                    this.OpCodes.JUMP_IF_FALSE_OR_POP_A, this.OpCodes.JUMP_IF_TRUE_OR_POP_A
                ].filter(v => v !== undefined));
                const sideEffect = new Set([
                    this.OpCodes.STORE_FAST_A, this.OpCodes.STORE_NAME_A, this.OpCodes.STORE_GLOBAL_A,
                    this.OpCodes.STORE_DEREF_A, this.OpCodes.STORE_ATTR_A, this.OpCodes.STORE_ATTR,
                    this.OpCodes.STORE_SUBSCR, this.OpCodes.STORE_SUBSCR_A, this.OpCodes.DELETE_FAST_A,
                    this.OpCodes.DELETE_NAME_A, this.OpCodes.DELETE_GLOBAL_A, this.OpCodes.RETURN_VALUE,
                    this.OpCodes.RETURN_VALUE_A, this.OpCodes.RETURN_CONST_A, this.OpCodes.RAISE_VARARGS_A,
                    this.OpCodes.IMPORT_NAME_A, this.OpCodes.YIELD_VALUE, this.OpCodes.YIELD_VALUE_A,
                    this.OpCodes.SETUP_FINALLY_A, this.OpCodes.SETUP_WITH_A, this.OpCodes.FOR_ITER_A,
                    this.OpCodes.POP_TOP, this.OpCodes.POP_TOP_A
                ].filter(v => v !== undefined));
                // Scan the fall-through path: a conditional test reached before any
                // side effect is an elif/else-if continuation (empty branch → pass);
                // a side effect first is plain loop body that a real continue skips.
                const insList = this.code.Instructions || [];
                let fi = this.code.GetIndexByOffset(this.curBlock.end);
                for (let steps = 0; fi >= 0 && fi < insList.length && steps < 16; fi++, steps++) {
                    const ins = insList[fi];
                    if (!ins) break;
                    if (condTest.has(ins.OpCodeID)) {
                        // A BACKWARD test (absolute era, <=3.10) followed by a
                        // forward JUMP_ABSOLUTE is the loop's bottom test
                        // (`if c: break` as last statement), not an elif header
                        // — the empty branch is a real `continue` that skips
                        // it. A backward test WITHOUT that break signature is a
                        // last-in-loop `elif` header (its false branch is
                        // peephole-retargeted to the loop top), and a forward
                        // test is an ordinary elif continuation — both mean the
                        // empty leading branch is `pass`.
                        if (this.object.Reader.versionCompare(3, 11) < 0 &&
                            [this.OpCodes.POP_JUMP_IF_FALSE_A,
                             this.OpCodes.POP_JUMP_IF_TRUE_A].includes(ins.OpCodeID)) {
                            let t = ins.Argument || 0;
                            if (this.object.Reader.versionCompare(3, 10) >= 0) t *= 2;
                            const nxt = insList[fi + 1];
                            if (t < ins.Offset &&
                                nxt && nxt.OpCodeID === this.OpCodes.JUMP_ABSOLUTE_A &&
                                (() => { let a = nxt.Argument || 0;
                                         if (this.object.Reader.versionCompare(3, 10) >= 0) a *= 2;
                                         return a > nxt.Offset; })()) {
                                break; // bottom test → keep continue
                            }
                        }
                        isContinue = false;
                        break;
                    }
                    if (sideEffect.has(ins.OpCodeID)) break; // real body → keep continue
                }
            }
            if (isContinue) {
    ...
    // 3.8/3.9 `break`: an unconditional JUMP_ABSOLUTE from inside the loop to
    // (or past) the loop's end. Mirrors the 3.11+ JUMP_FORWARD break branch in
    // processJumpForward. Natural intra-loop jumps (if/else merges, try
    // success paths, continues) all target the loop top or a merge point
    // BEFORE the loop end, never at/past it. `>=` because a top-tested
    // while's block end IS the exit offset the break jumps to (a bottom-
    // tested `while True:` ends at its loop-back, which the break jumps
    // over). Without this the jump collapses into a spurious `else` that
    // swallows the rest of the loop body.
    if (this.object.Reader.versionCompare(3, 8) >= 0 &&
        this.object.Reader.versionCompare(3, 10) < 0) {
        let loop = null;
        for (let bi = this.blocks.length - 1; bi >= 0; bi--) {
        ...
        if (loop && loop.end > 0 && this.code.Current.Offset < loop.end && offs >= loop.end) {
...
function handleJumpBackwardA() {
    ...
    // A back-edge from inside a body conditional is a `continue` when it is not
    // the loop's own re-entry: either it targets above the loop body start (a
    // duplicated-condition while re-tests at the top), or a STRICTLY LATER
    // back-edge lands on the same target — the later one is the real loop-back, so
    // this earlier jump skips the rest of the body. (loop.start can be reported at
    // the code top rather than the condition top, e.g. on 3.14, so the farthest-
    // back-edge test must not be gated on target === loop.start.)
    let isContinue = target < loop.start;
    if (!isContinue && typeof this.getBackJumpMap === 'function') {
...
// Python 3.11 conditional backward jumps (POP_JUMP_BACKWARD_IF_TRUE/FALSE).
// These are the loop-back re-test of a duplicated-condition `while`: when the
// while is reconstructed, the whole bottom copy sits in a chainDeadRegion and is
// skipped before dispatch, so this handler only runs for loop shapes we did NOT
// reconstruct (e.g. a chained comparison inside a `while`, tracked in #14).
// Behave like a conditional loop-back: pop the tested condition to keep the data
// stack balanced, then reuse JUMP_BACKWARD's continue detection. This yields
// valid best-effort Python instead of an "unsupported opcode" abort.
function handleConditionalJumpBackward() {
    if (this.dataStack.length > 0) this.dataStack.pop();
    handleJumpBackwardA.call(this);
}

function handlePopJumpBackwardIfTrueA() {
    handleConditionalJumpBackward.call(this);
}

function handlePopJumpBackwardIfFalseA() {
    handleConditionalJumpBackward.call(this);
}
...
module.exports = {
    ...
    handlePopJumpBackwardIfTrueA,
    handlePopJumpBackwardIfFalseA,
