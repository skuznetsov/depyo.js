...
function processJumpOps() {
    ...
    if (cond instanceof AST.ASTCompare
        ...
        if (global.g_cliArgs?.debug) {
            console.log(`  EXCEPTION MATCH detected: Creating Except block with condition=${cond.right?.constructor.name} ${cond.right?.codeFragment?.()}`);
        }
        ...
        if (this.curBlock.blockType == AST.ASTBlock.BlockType.Except
            ...
            this.curBlock.condition = cond.right;
            if (handlerEnd) {
            ...
            ifblk = new AST.ASTCondBlock(AST.ASTBlock.BlockType.Except, this.code.Current.Offset, end, cond.right, false);
