...
function consumeChainedCompareEpilogue(ifblk) {
    ...
    if (!jf || jf.OpCodeID !== this.OpCodes.JUMP_FORWARD_A) return;
    if (!popTop || ![this.OpCodes.POP_TOP, this.OpCodes.POP_TOP_A].includes(popTop.OpCodeID)) return;
    ...
    // Leave Current just before the body so the main loop's GoNext lands on it;
    // the epilogue (JUMP_FORWARD/POP_TOP/[JUMP_FORWARD]) is skipped as machinery.
