...
function handleMatchSequence() {
    ...
    if (!this.currentMatch) {
        // Try to get subject from dataStack first (normal case)
        ...
        // Fallback 1: try TOS if TOS1 undefined
        if (!this.matchSubject && this.dataStack.length > 0) {
            this.matchSubject = this.dataStack.top();
            if (global.g_cliArgs?.debug) {
                console.log(`  Subject from TOS-1 undefined, trying TOS`);
            }
        }

        // Fallback 2: use potentialMatchSubject (saved at COPY after LOAD)
        // This survives dataStack changes from block creation
        if (!this.matchSubject && this.potentialMatchSubject) {
            this.matchSubject = this.potentialMatchSubject;
            if (global.g_cliArgs?.debug) {
                console.log(`  Subject from dataStack undefined, using saved potentialMatchSubject`);
            }
        }
