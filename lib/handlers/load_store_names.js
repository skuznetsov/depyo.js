...
function processStore() {
    // Record pattern operation if in pattern matching (variable capture)
    // In pattern matching, STORE_FAST is ALWAYS a variable capture (not normal unpacking)
    // even if this.unpack > 0 from UNPACK_SEQUENCE
    if (this.inMatchPattern) {
        this.patternOps.push({
            type: 'STORE_FAST',
            name: this.code.Current.Name
        });
        // Don't process normal store logic during pattern matching
        // The STORE is just recording which variable captures the value
        return;
    }
