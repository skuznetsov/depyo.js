...
function handleLoadCommonConstantA() {
    // Python 3.14 LOAD_COMMON_CONSTANT (replaces LOAD_ASSERTION_ERROR). The
    // common-constants table is exception classes, not values — CPython's
    // _common_constants is [AssertionError, NotImplementedError]. The previous
    // None/False/True/int mapping was a wrong guess and made `assert` render
    // `raise None`.
    const COMMON = ["AssertionError", "NotImplementedError"];
    const name = COMMON[this.code.Current.Argument] ?? `__common_const_${this.code.Current.Argument}__`;
    let node = new AST.ASTName(name);
    node.line = this.code.Current.LineNo;
    this.dataStack.push(node);
}
