...
function handleLoadSpecialA() {
    // Python 3.14 LOAD_SPECIAL (e.g., __enter__/__exit__ on context manager): treat like LOAD_ATTR on TOS.
    let obj = this.dataStack.pop();
    const attrName = this.code.Current.Name || "";
    if (!obj) {
        obj = new AST.ASTName("##SPECIAL##");
    }
    let node = new AST.ASTBinary(obj, new AST.ASTName(attrName), AST.ASTBinary.BinOp.Attr);
    node.line = this.code.Current.LineNo;
    this.dataStack.push(node);
}
