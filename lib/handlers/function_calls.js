...
function handleInstrumentedCallA() {
    ...
    if (loadBuildClassFound) {
        ...
        if (this.object.Reader.versionCompare(3, 6) < 0) {
            ...
            // CALL_FUNCTION arg encodes pparams (low byte) and kwparams (high byte).
            // pparams includes function + name + bases; kwparams covers metaclass etc.
            for (let i = 0; i < kwparams; i++) {
                let value = this.dataStack.pop();
                let key = this.dataStack.pop();
                lbcKwparamList.unshift({key, value});
            }
            for (let i = 0; i < pparams - 2; i++) {
                bases.unshift(this.dataStack.pop());
            }
        } else {
            // 3.6+: kwargs flow via CALL_FUNCTION_KW or KW_NAMES (separate paths),
            // so all stack entries above name/function are bases.
