...
function handlePopExcept() {
    ...
    if (this.curBlock.blockType == AST.ASTBlock.BlockType.Try) {
        const isExceptStar = !!this.inExceptionGroup;
        let catchBlock = new AST.ASTExceptBlock(exceptBlock.start, exceptBlock.end, this.curBlock);
        catchBlock.line = exceptBlock.line;
        catchBlock.expr = exceptBlock.condition;
        catchBlock.inited = true;
        catchBlock.nodes = bodyNode.nodes;
        catchBlock.isExceptStar = isExceptStar;
        this.curBlock.addExcept(catchBlock);
...
function handleReraise() {
    ...
    if (this.curBlock.blockType === AST.ASTBlock.BlockType.Except) {
        ...
        this.curBlock = this.blocks.top();
        if (this.curBlock.blockType === AST.ASTBlock.BlockType.Try) {
            let catchBlock = new AST.ASTExceptBlock(excBlock.start, excBlock.end, this.curBlock);
            catchBlock.line = excBlock.line;
            catchBlock.expr = excBlock.condition;
            catchBlock.inited = true;
            catchBlock.nodes = excBlock.nodes;
...
function handlePushExcInfo() {
    ...
    // Treat as a marker: the next POP_EXCEPT belongs to except* handler.
