...
function handleSetupFinallyA() {
    ...
    if (withPattern) {
        ...
        // Normal SETUP_FINALLY for try/finally blocks
...
function handleEndFinally() {
    ...
    if (this.curBlock.blockType == AST.ASTBlock.BlockType.Container) {
        ...
        // Check if parent is an uninitialized AsyncFor block
        if (this.blocks.length > 1) {
            ...
            if (global.g_cliArgs?.debug) {
                console.log(`[handleEndFinally-Container] Parent=${parentBlock?.type_str}, isUninitAsyncFor=${isUninitAsyncFor}`);
            }
