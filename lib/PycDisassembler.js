...
{
    static Disassemble (obj, parentPrefix) {
        ...
        while (code.HasInstructionsToProcess) {
            try {
                ...
                result += "\t" + [pad(opCode.Offset, 5), pad(opCode.InstructionName, 13), pad(opCode.HasArgument ? opCode.Argument : "", 4), " ", argValue != "" ? "(" + argValue + ")" : ""].join(" ") + "\n";
            } catch (ex) {
        ...
        for (let code_obj of obj.Consts.Value.filter(o => o instanceof PythonCodeObject)) {
            result += "\n";
            result += PycDisassembler.Disassemble(code_obj, methodName);
        }
