...
class PycDecompiler {
    ...
    cleanupExcMatchArtifacts(root) {
        ...
        const isEmptyTryBlock = (node) =>
            ...
            (node.nodes.length === 0 ||
             (node.nodes.length === 1 &&
              ...
              node.nodes[0].word === AST.ASTKeyword.Word.Pass));
        ...
        const rewriteContainer = (nodes, parentBlockType) => {
            ...
            // Pass 4: drop empty `try: pass` blocks with no following handler.
            for (let i = nodes.length - 1; i >= 0; i--) {
            ...
            // produces SyntaxError at parse time. Skip when parent is Try:
            // except* groups legitimately nest Except inside Try bodies.
            if (parentBlockType === AST.ASTBlock.BlockType.Try) return;
            for (let i = nodes.length - 1; i >= 0; i--) {
                const cur = nodes[i];
        ...
        // Traverse: ASTNodeList.list and ASTBlock.nodes are mutable arrays.
        const queue = [root];
        const visited = new WeakSet();
        while (queue.length) {
            const n = queue.shift();
            ...
            if (n instanceof AST.ASTNodeList) {
                rewriteContainer(n.list, null);
                children = n.list;
            } else if (n instanceof AST.ASTBlock) {
                rewriteContainer(n.nodes, n.blockType);
                children = n.nodes;
            } else if (n instanceof AST.ASTStore && n.src instanceof AST.ASTFunction) {
                children = [n.src.code?.object?.SourceCode];
            }
            if (Array.isArray(children)) {
                for (const c of children) if (c) queue.push(c);
            }
        }
    }
