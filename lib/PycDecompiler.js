...
class PycDecompiler {
    ...
    static Decompile (obj, depth = 0) {
        ...
        let funcBody = PycDecompiler.Stmts(depth, obj, code, DataStack);
    ...
    static Stmts (depth, obj, code, DataStack, endOffset = 0) {
        let nextOpCode = null;
        ...
        let result = new PycResult(null, depth);
            ...
            try {
                ...
                switch (code.Current.OpCodeID)
                {
                    ...
                    case OpCodes.POP_JUMP_IF_FALSE:
                        PycDecompiler.DecompileIfExpression(depth, obj, code, DataStack, result);
                        break;
                    case OpCodes.POP_JUMP_IF_TRUE:
                        ...
                        if (raiseOffset > 0 && nextOpCode.OpCodeID == OpCodes.LOAD_GLOBAL) {
                            ...
                            if (raiseOp.Argument == 2) {
                                msg = PycDecompiler.Expr(depth, obj, code, DataStack, raiseOffset);
                            }
                    ...
                    case OpCodes.SETUP_LOOP:
                        ...
                        if (forOffset >= 0 && forOffset < jumpOffset && jumpOffset < jumpTarget) {
                            let inExpr = PycDecompiler.Expr(depth, obj, code, DataStack, getIterOffset);
                            code.GoNext(2);
                            let forJump = code.Current.JumpTarget;
                            let forExpr = PycDecompiler.Expr(depth, obj, code, DataStack);
                            result.Add("for " + forExpr + " in " + inExpr + ":");
                            let forBody = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, forJump);
                            result.Add(forBody);

                            if (forJump < jumpTarget) {
                                code.GoNext();
                                if (code.Current.Argument > 0) {
                                    let elseBody = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, jumpTarget);
                            ...
                            if (ifOpLine == whileLine) {
                                testCondition = PycDecompiler.Expr(depth, obj, code, DataStack, ifOffset);
                                // Skipping POP_JUMP_IF_ command as we need only test condition for it
                                code.GoNext();
                                [testConditionExtra, jumpOffset] = PycDecompiler.ExtractTestCondition(depth, obj, code, DataStack, jumpOffset);
                            ...
                            let whileBody = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, jumpOffset);
                    ...
                    case OpCodes.SETUP_EXCEPT:
                        {
                            ...
                            let bodyStmt = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, jumpTarget - 4);
                            result.Add(bodyStmt);
                            ...
                            while (true) {
                                ...
                                if (code.Next.OpCodeID == OpCodes.DUP_TOP) {
                                    // except handler type
                                    code.GoNext();
                                    exceptHandlerType = PycDecompiler.Expr(depth, obj, code, DataStack, code.GetOffsetByOpCode(OpCodes.COMPARE_OP));
                                    excJump = code.GetNextInstruction(2).Argument;
                                }
                                ...
                                if (nextOpCode.OpCodeID != OpCodes.POP_TOP) {
                                    varName = PycDecompiler.Expr(depth, obj, code, DataStack, code.GetOffsetByOpCode(OpCodes.POP_TOP));
                                } else {
                                    code.GoNext();
                                }
                                ...
                                excBody = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, excJump == 0 ? elseJump : excJump);
                                ...
                                if (code.PeekInstructionAtOffset(excJump).OpCodeID == OpCodes.END_FINALLY) {
                                    code.GoToOffset(excJump + 1); // after END_FINALLY
                                    break;
                                }
                            }
                            if (code.Current.Offset + 1 < endJump) {
                                let elseBody = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, endJump);
                                if (elseBody.HasResult) {
                                    result.Add("else:");
                                    result.Add(elseBody);
                                }
                    ...
                    case OpCodes.SETUP_FINALLY:
                        ...
                        let bodyStmts = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, finallyBodyAddress - 4);
                        code.GoNext(2);
                        let endFinallyAddress = code.GetOffsetByOpCode(OpCodes.END_FINALLY, finallyBodyAddress);
                        let finallyBodyStmts = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, endFinallyAddress);
                        result.Add("try:");
                    ...
                    case OpCodes.MAKE_CLOSURE:
                        ...
                        if (child_co.Name == "<lambda>") {
                            PycDecompiler.MakeLambda(depth + 1, child_co, DataStack, default_params);
                            break;
                        }
                        ...
                        } else if (code.Prev.InstructionName.startsWith("LOAD_") && DataStack.length > 0) {
                    ...
                    default:
                        ...
                        let res = PycDecompiler.Expr(depth, obj, code, DataStack, endOffset);
                        if (res != "") {
                            result.Add(res);
                        }
                        break;
    ...
    static MakeLambda(depth, child_co, DataStack, default_params) {
        let code = new OpCodes(child_co);
        let source = PycDecompiler.Stmts(depth, child_co, code, DataStack);
        let pars = "";
    ...
    static DecompileIfExpression(depth, obj, code, DataStack, result) {
        ...
        let elseOffset = (jumpOpcode.OpCodeID == OpCodes.JUMP_FORWARD && jumpOpcode.Argument > 0) ? jumpOpcode.JumpTarget : null;
        ...
        [ifTestPart, jumpOffset] = PycDecompiler.ExtractTestCondition(depth, obj, code, DataStack, jumpOffset);
        ifTest += ifTestPart;

        ifBody = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, jumpOffset);
        ...
        if (code.Current.OpCodeID == OpCodes.JUMP_FORWARD) {
            if (code.Current.Offset < jumpOffset && code.Current.Argument > 0) {
                let jumpTarget = code.Current.JumpTarget;
                ifElseBody = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, jumpTarget);
            }
        }
    ...
    static FindIfCodesWithTheSameTarget(obj, code, offset = -1, endOffset = -1) {
        let startPos = offset == -1 ? 0 : code.GetIndexByOffset(offset);
        let endPos = endOffset == -1 ? code.Instructions.length : code.GetIndexByOffset(endOffset);
        let commonTarget = -1;
        let result = [];
        let prevOffset = 0;
        ...
        for (let position = startPos; position < endPos; position++) {
            ...
            if ([OpCodes.POP_JUMP_IF_FALSE, OpCodes.POP_JUMP_IF_TRUE, OpCodes.JUMP_IF_FALSE_OR_POP, OpCodes.JUMP_IF_TRUE_OR_POP].includes(currentOp.OpCodeID)) {
                ...
                } else if (currentOp.JumpTarget != commonTarget || obj.LineNoTab[prevOffset] != currentOp.Offset) {
                    break;
                }
    ...
    static ExtractTestCondition(depth, obj, code, DataStack, endOffset) {
        let ifTest = "";
        let jumpOffset = endOffset;

        if (endOffset < code.Current.Offset) {
            let backOpCode = code.PeekInstructionAtOffset(endOffset);
            if (backOpCode.HasJumpAbsolute || backOpCode.HasJumpRelative) {
                endOffset = backOpCode.JumpTarget;
                jumpOffset = endOffset;
            }
        }
        ...
        if ( ifOffests.length > 1 && code.Current.Offset < endOffset) {
            ...
            for (let ifOffset of ifOffests) {
                ...
                if ([OpCodes.JUMP_IF_FALSE_OR_POP, OpCodes.POP_JUMP_IF_FALSE].includes(code.Current.OpCodeID))
                    ...
                    op = " or ";
                let temp = PycDecompiler.Expr(depth, obj, code, DataStack, ifOffset);
                if (temp == "")
                {
                    temp = DataStack.pop();
                }
    ...
    static Expr (depth, obj, code, DataStack, endOffset = 0) {
            ...
            try {
                ...
                switch (code.Current.OpCodeID) {
                    ...
                    case OpCodes.GET_ITER:
                        ...
                        {
                            code.GoNext(DataStack.length > 1 ? 2 : 1);
                    ...
                    case OpCodes.FOR_ITER:
                        nextOpCode = code.Prev;
                        if (nextOpCode.OpCodeID == OpCodes.LOAD_FAST && nextOpCode.Argument == 0) {
                            if (["<genexpr>", "<dictcomp>", "<setcomp>"].includes(obj.Name)) {
                                DataStack.pop();
                                let target = PycDecompiler.Expr(depth, obj, code, DataStack);
                                let ifExpr = PycDecompiler.Expr(depth, obj, code, DataStack);
                                let yieldExpr = "";
                                ...
                                if (ifExpr.startsWith("yield ")) {
                                    ...
                                    yieldExpr = PycDecompiler.Expr(depth, obj, code, DataStack);
                                }
                                ...
                                let ifExpr = PycDecompiler.Expr(depth, obj, code, DataStack);
                                code.GoNext();
                                let yieldExpr = PycDecompiler.Expr(depth, obj, code, DataStack).replace(/yield /g, "");
                                code.GoNext(2);
                    ...
                    case OpCodes.BUILD_LIST:
                        {
                            ...
                            if (code.Current.Argument == 0 && (forOffset == -1 || (setupLoopOffset > 0 && forOffset > setupLoopOffset) || (forOffset - code.Current.Offset) > 35)) {
                                ...
                                if (forOffset >= 0 && forOffset < jumpOffset) {
                                    let iter = PycDecompiler.Expr(depth, obj, code, DataStack, getIterOffset);
                                    code.GoNext(2);
                                    let target = PycDecompiler.Expr(depth, obj, code, DataStack);
                                ...
                                if (ifOffset >= 0 && ifOffset <= jumpOffset) {
                                    ifExpr = " if " + PycDecompiler.Expr(depth, obj, code, DataStack, ifOffset);
                                    code.GoNext();
                                }

                                let listAppendOffset = code.GetOffsetByOpCode(OpCodes.LIST_APPEND);
                                let expr = PycDecompiler.Expr(depth, obj, code, DataStack, listAppendOffset);
                                code.GoNext(2);
                    ...
                    case OpCodes.BUILD_MAP:
                        {
                            ...
                            if (code.Current.Argument == 0 && (forOffset == -1 || (setupLoopOffset > 0 && forOffset > setupLoopOffset) || (forOffset - code.Current.Offset) > 15)) {
                                ...
                                if (forOffset >= 0 && forOffset < jumpOffset) {
                                    let iter = PycDecompiler.Expr(depth, obj, code, DataStack, forOffset);
                                    code.GoNext();
                                    let target = PycDecompiler.Expr(depth, obj, code, DataStack);
                                    comp_for = `for ${target} in ${iter}`;
                                }
                                ...
                                let key = PycDecompiler.Expr(depth, obj, code, DataStack, mapAddOffset);
                                let value = PycDecompiler.Expr(depth, obj, code, DataStack, mapAddOffset);
                                code.GoNext(2);
                                ...
                                while (count-- > 0) {
                                    let expr = PycDecompiler.Expr(depth, obj, code, DataStack);
                    ...
                    case OpCodes.IMPORT_NAME:
                        {
                            ...
                            if (nextOpCode.OpCodeID == OpCodes.IMPORT_STAR) {
                                ...
                                if (names.ClassName == "Py_Tuple") {
                                    ...
                                    let name = PycDecompiler.Expr(depth, obj, code, DataStack, code.GetOffsetByOpCodeName("STORE_") + 3);
                    ...
                    case OpCodes.JUMP_IF_TRUE_OR_POP:
                        {
                            ...
                            for (let offset of ifsOffs) {
                                resultlet += PycDecompiler.Expr(depth, obj, code, DataStack, offset);
                                code.GoNext();
                            ...
                            if (jumpForward < jumpTarget &&
                                ...
                                resultlet += PycDecompiler.Expr(depth, obj, code, DataStack, jumpForward);
                                code.GoNext(3);
                            } else {
                                resultlet += PycDecompiler.Expr(depth, obj, code, DataStack, jumpTarget);
                            }
                    ...
                    case OpCodes.SETUP_WITH:
                        ...
                        let bodyStmts = PycDecompiler.Stmts(depth + 1, obj, code, DataStack, jumpOffset);
                        bodyStmts.Result.pop();
                        result.DoNotIndent = true;
                        result.Add(`with ${cond} as ${withName}:`);
                        result.Add(bodyStmts);
                        result.Add("");
                        return result;
