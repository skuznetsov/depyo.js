...
class PycDecompiler {
    ...
    // Resolve the target of an unconditional backward jump (JUMP_ABSOLUTE on
    // <=3.10, JUMP_BACKWARD/NO_INTERRUPT on 3.11+). resolveJumpTarget-style
    // helpers only add the delta; backward jumps subtract, so compute here.
    computeBackJumpTarget(instr, idx) {
        const reader = this.object.Reader;
        let arg = instr.Argument || 0;
        if (reader.versionCompare(3, 10) >= 0) arg *= 2; // BPO-27129 wordcode doubling
        if (instr.OpCodeID === this.OpCodes.JUMP_ABSOLUTE_A) {
            return arg; // absolute (already *2 on 3.10)
        }
        // JUMP_BACKWARD / JUMP_BACKWARD_NO_INTERRUPT: base = next real offset.
        const instrs = this.code.Instructions || [];
        let j = idx + 1;
        while (j < instrs.length && instrs[j] && instrs[j].OpCodeID === this.OpCodes.CACHE) j++;
        const base = (j < instrs.length && instrs[j]) ? instrs[j].Offset : (instr.Offset + (instr.Size || 2));
        return base - arg;
    }

    // Resolve a jump's target if (and only if) it is a BACKWARD edge; else null.
    // Covers every loop-back / continue shape used across 3.10-3.13:
    //   - JUMP_ABSOLUTE (3.10) and pre-3.12 POP_JUMP_IF_TRUE/FALSE are absolute
    //     (arg*2) and can point backward;
    //   - JUMP_BACKWARD / JUMP_BACKWARD_NO_INTERRUPT / POP_JUMP_BACKWARD_IF_* are
    //     backward-relative (base - arg*2).
    // 3.12+ POP_JUMP_IF_* are forward-relative and never a back-edge, so they are
    // intentionally excluded.
    resolveBackEdgeTarget(instr, idx) {
        const OpCodes = this.OpCodes;
        const reader = this.object.Reader;
        const op = instr.OpCodeID;
        let arg = (instr.Argument || 0);
        if (reader.versionCompare(3, 10) >= 0) arg *= 2;
        const absoluteOps = [OpCodes.JUMP_ABSOLUTE_A];
        if (reader.versionCompare(3, 12) < 0) {
            absoluteOps.push(OpCodes.POP_JUMP_IF_TRUE_A, OpCodes.POP_JUMP_IF_FALSE_A);
        }
        const backwardRelOps = [OpCodes.JUMP_BACKWARD_A, OpCodes.JUMP_BACKWARD_NO_INTERRUPT_A,
                                OpCodes.POP_JUMP_BACKWARD_IF_TRUE_A, OpCodes.POP_JUMP_BACKWARD_IF_FALSE_A]
                               .filter(v => v !== undefined);
        let tgt;
        if (absoluteOps.includes(op)) {
            tgt = arg;
        } else if (backwardRelOps.includes(op)) {
            const instrs = this.code.Instructions || [];
            let j = idx + 1;
            while (j < instrs.length && instrs[j] && instrs[j].OpCodeID === OpCodes.CACHE) j++;
            const base = (j < instrs.length && instrs[j]) ? instrs[j].Offset : (instr.Offset + (instr.Size || 2));
            tgt = base - arg;
        } else {
            return null;
        }
        return tgt < instr.Offset ? tgt : null; // only backward edges
    }

    // Map each backward-jump target offset -> the FARTHEST backward jump that
    // lands on it (the true loop-back; earlier ones are `continue`s). Cached per
    // code reader. Used to recognise `while True:` loops that have no entry guard.
    getBackJumpMap() {
        if (this.code._backJumpMap) return this.code._backJumpMap;
        const map = new Map();
        const instrs = this.code.Instructions || [];
        const backOps = [this.OpCodes.JUMP_ABSOLUTE_A, this.OpCodes.JUMP_BACKWARD_A,
                         this.OpCodes.JUMP_BACKWARD_NO_INTERRUPT_A];
        for (let i = 0; i < instrs.length; i++) {
            const ins = instrs[i];
            if (!ins || !backOps.includes(ins.OpCodeID)) continue;
            const tgt = this.computeBackJumpTarget(ins, i);
            if (tgt >= ins.Offset) continue; // must be backward
            const ex = map.get(tgt);
            if (!ex || ins.Offset > ex.offset) map.set(tgt, { offset: ins.Offset, instr: ins, idx: i });
        }
        this.code._backJumpMap = map;
        return map;
    }

    // 3.8/3.9 bottom-tested `while True:`. When `if c: break` is the LAST
    // statement of a `while True:` body, the peephole rewrites it into a
    // BACKWARD conditional jump to the loop top (cond false -> next iteration)
    // followed by the break's JUMP_ABSOLUTE to the exit and a dead
    // JUMP_ABSOLUTE loop-back:
    //
    //   T:  <body>
    //       <c>; POP_JUMP_IF_FALSE T     # J: bottom test (backward conditional)
    //       JUMP_ABSOLUTE exit           # F: the break
    //       JUMP_ABSOLUTE T              # dead loop-back
    //   exit:
    //
    // There is no entry guard, so the loop is invisible to the pre-3.10 loop
    // machinery and the body unrolls (`if c: pass`, loop dropped). Map each
    // such shape's loop-top offset T; maybeOpenBottomTestedLoop opens the
    // While when the PC reaches T. Cached per code object.
    getBottomTestedLoopMap() {
        if (this.code._bottomTestedLoopMap) return this.code._bottomTestedLoopMap;
        const map = new Map();
        const reader = this.object?.Reader;
        if (reader && reader.versionCompare(3, 8) >= 0 && reader.versionCompare(3, 10) < 0) {
            const instrs = this.code.Instructions || [];
            for (let i = 0; i < instrs.length; i++) {
                const ins = instrs[i];
                if (!ins) continue;
                // Kind 'cond': the bottom test itself loops back (if-break last).
                if (ins.OpCodeID === this.OpCodes.POP_JUMP_IF_FALSE_A ||
                    ins.OpCodeID === this.OpCodes.POP_JUMP_IF_TRUE_A) {
                    const tgt = ins.Argument || 0; // absolute byte offset on <=3.9
                    if (tgt < ins.Offset) { // backward: the bottom test
                        const nxt = instrs[i + 1];
                        if (!nxt || nxt.OpCodeID !== this.OpCodes.JUMP_ABSOLUTE_A) continue;
                        if ((nxt.Argument || 0) <= nxt.Offset) continue; // the break jumps FORWARD to the exit
                        const ex = map.get(tgt);
                        if (!ex || ins.Offset > ex.jOffset) {
                            map.set(tgt, { kind: 'cond', jOffset: ins.Offset, fOffset: nxt.Offset });
                        }
                        continue;
                    }
                    // Kind 'condexit': the peephole inverted the bottom test
                    // into a FORWARD conditional exit sitting directly over the
                    // loop-back (`…; if not c: continue; break` folds to this).
                    // Guard against the identical-looking top-tested header
                    // (`while cond:`/walrus, incl. empty body): the guard must
                    // sit on a LATER source line than the loop top and real
                    // body (a side effect) must precede it.
                    const nxt = instrs[i + 1];
                    if (!nxt || nxt.OpCodeID !== this.OpCodes.JUMP_ABSOLUTE_A) continue;
                    const T2 = nxt.Argument || 0;
                    if (T2 >= nxt.Offset) continue;  // must loop back
                    if (tgt <= nxt.Offset) continue; // exit must jump past the loop-back
                    const topInstr = instrs.find(x => x && x.Offset === T2);
                    if (!topInstr || !(ins.LineNo > topInstr.LineNo)) continue;
                    const bodyEffect = new Set([
                        this.OpCodes.STORE_FAST_A, this.OpCodes.STORE_NAME_A,
                        this.OpCodes.STORE_GLOBAL_A, this.OpCodes.STORE_DEREF_A,
                        this.OpCodes.STORE_ATTR_A, this.OpCodes.STORE_ATTR,
                        this.OpCodes.STORE_SUBSCR, this.OpCodes.STORE_SUBSCR_A,
                        this.OpCodes.POP_TOP, this.OpCodes.POP_TOP_A, this.OpCodes.PRINT_EXPR
                    ].filter(v => v !== undefined));
                    let hasBody = false;
                    for (let k = 0; k < instrs.length; k++) {
                        const bd = instrs[k];
                        if (!bd || bd.Offset < T2) continue;
                        if (bd.Offset >= ins.Offset) break;
                        if (bodyEffect.has(bd.OpCodeID)) { hasBody = true; break; }
                    }
                    if (!hasBody) continue;
                    const ex = map.get(T2);
                    if (!ex || nxt.Offset > ex.jOffset) {
                        map.set(T2, { kind: 'condexit', jOffset: nxt.Offset,
                                      jSize: nxt.Size || 2, guardOffset: ins.Offset });
                    }
                    continue;
                }
                // Kind 'uncond': plain unconditional loop-back (`while True:` with
                // the break somewhere mid-body). Only registered when there is
                // break EVIDENCE: an unconditional JUMP_ABSOLUTE inside (T, J)
                // jumping past J. Without an exit the loop stays invisible here
                // (extending detection to every unguarded back-edge on 3.8/3.9
                // regressed async-for/walrus shapes).
                if (ins.OpCodeID === this.OpCodes.JUMP_ABSOLUTE_A) {
                    const tgt = ins.Argument || 0;
                    if (tgt >= ins.Offset) continue;
                    // Skip the peephole's UNREACHABLE trailing loop-back (it
                    // follows another jump/terminator); registering it would
                    // shadow the reachable 'cond'/'condexit' entry for T.
                    const prev = instrs[i - 1];
                    if (prev && [this.OpCodes.JUMP_ABSOLUTE_A, this.OpCodes.JUMP_FORWARD_A,
                                 this.OpCodes.RETURN_VALUE, this.OpCodes.RETURN_VALUE_A,
                                 this.OpCodes.RAISE_VARARGS_A].filter(v => v !== undefined)
                                 .includes(prev.OpCodeID)) continue;
                    let hasBreak = false;
                    for (let k = 0; k < instrs.length; k++) {
                        const bk = instrs[k];
                        if (!bk || bk.Offset <= tgt) continue;
                        if (bk.Offset >= ins.Offset) break;
                        if (bk.OpCodeID === this.OpCodes.JUMP_ABSOLUTE_A &&
                            (bk.Argument || 0) > ins.Offset) { hasBreak = true; break; }
                    }
                    if (!hasBreak) continue;
                    const ex = map.get(tgt);
                    if (!ex || ins.Offset > ex.jOffset) {
                        map.set(tgt, { kind: 'uncond', jOffset: ins.Offset, jSize: ins.Size || 2 });
                    }
                }
            }
        }
        this.code._bottomTestedLoopMap = map;
        return map;
    }

    maybeOpenBottomTestedLoop() {
        const reader = this.object?.Reader;
        if (!reader || reader.versionCompare(3, 8) < 0 || reader.versionCompare(3, 10) >= 0) return;
        const cur = this.code.Current;
        if (!cur) return;
        const T = cur.Offset;
        const entry = this.getBottomTestedLoopMap().get(T);
        if (!entry) return;
        const BT = AST.ASTBlock.BlockType;
        // A loop already starts here (top-tested while / for) — don't double-open.
        if (this.blocks.some(b => [BT.While, BT.For, BT.AsyncFor].includes(b.blockType) && b.start === T)) return;
        if (cur.OpCodeID === this.OpCodes.FOR_ITER_A) return;

        const instrs = this.code.Instructions || [];
        // A guarded loop's condition — top-tested `while COND:` (including a
        // walrus in the header, whose STORE runs before the guard) — contains
        // a pop-jump to the loop EXIT (past the bottom back-edge). A genuine
        // `while True:` body never does: its `if c: break` compiles to a
        // conditional over the break's JUMP_ABSOLUTE (target <= J) or to the
        // backward bottom test itself. Any exit guard in [T, J) means this
        // loop belongs to the conditional-while machinery.
        for (let i = this.code.CurrentInstructionIndex; i < instrs.length; i++) {
            const ins = instrs[i];
            if (!ins || ins.Offset >= entry.jOffset) break;
            if ((ins.OpCodeID === this.OpCodes.POP_JUMP_IF_FALSE_A ||
                 ins.OpCodeID === this.OpCodes.POP_JUMP_IF_TRUE_A) &&
                (ins.Argument || 0) > entry.jOffset &&
                ins.Offset !== entry.guardOffset) { // condexit's own exit guard is the break
                return;
            }
            // Async-for loop tops also arrive as unguarded back-edge targets on
            // 3.8/3.9; their machinery must keep them.
            if ([this.OpCodes.GET_ANEXT, this.OpCodes.GET_AITER,
                 this.OpCodes.END_ASYNC_FOR].filter(v => v !== undefined)
                 .includes(ins.OpCodeID)) {
                return;
            }
        }
        // Reject fragments: a back-edge from after the bottom test into the
        // body interior means [T, J] is part of a larger loop, not a
        // self-contained `while True:`. (The dead loop-back targets T itself.)
        for (let i = 0; i < instrs.length; i++) {
            const ins = instrs[i];
            if (!ins || ins.Offset <= entry.jOffset) continue;
            const t2 = this.resolveBackEdgeTarget(ins, i);
            if (t2 !== null && t2 > T && t2 <= entry.jOffset) return;
        }

        // Kind 'cond' ends at the break's JUMP_ABSOLUTE (the bottom test stays
        // inside as `if c: break`, emitted by processJumpOps); kind 'uncond'
        // ends at the unconditional loop-back, which is skipped like
        // maybeOpenInfiniteLoop does.
        const endOffset = entry.kind === 'cond' ? entry.fOffset : entry.jOffset;
        const wblk = new AST.ASTCondBlock(BT.While, T, endOffset, null, false);
        wblk.line = cur.LineNo;
        wblk.init();
        this.blocks.push(wblk);
        this.curBlock = wblk;
        if (entry.kind === 'uncond' || entry.kind === 'condexit') {
            this.addDeadRegion(entry.jOffset, entry.jOffset + (entry.jSize || 2));
        }
        // (kind 'cond': the break jump + dead loop-back region is registered by
        // the processJumpOps bottom-test branch, which knows both jumps.)
    }

    // Register a [from, to) range the main loop must skip, unless an existing
    // region already covers its start.
    addDeadRegion(from, to) {
        this.chainDeadRegions ||= [];
        if (this.chainDeadRegions.some(r => from >= r.from && from < r.to)) return;
        this.chainDeadRegions.push({ from, to });
    }

    // Detect a `while True:` (constant-condition / infinite) loop at the current
    // offset and open a While block for it. CPython emits no entry guard for such
    // loops: the body is entered by fall-through and only an unconditional
    // backward jump at the bottom loops back to the top. Conditional `while COND:`
    // loops (3.10-3.13 duplicated-condition, 3.9 top-tested, 3.14 single) always
    // open their While at the guard *before* the body, so a loop already starts
    // here in those cases; and their body-start is immediately preceded by the
    // entry-guard pop-jump to the loop exit. Both facts are checked to avoid
    // misfiring. Scoped to 3.10+ (older loop shapes are handled elsewhere).
    maybeOpenInfiniteLoop() {
        const reader = this.object?.Reader;
        if (!reader || reader.versionCompare(3, 10) < 0) return;
        const cur = this.code.Current;
        if (!cur) return;
        const T = cur.Offset;
        const BT = AST.ASTBlock.BlockType;
        // A loop already starts here (conditional while / for) — don't double-open.
        if (this.blocks.some(b => [BT.While, BT.For, BT.AsyncFor].includes(b.blockType) && b.start === T)) return;
        // A for-loop top is FOR_ITER, not an infinite while.
        if (cur.OpCodeID === this.OpCodes.FOR_ITER_A ||
            cur.OpCodeID === this.OpCodes.INSTRUMENTED_FOR_ITER_A) return;

        const back = this.getBackJumpMap().get(T);
        if (!back) return;
        const J = back.instr;

        // Reject `continue` targets masquerading as loop tops. A real `while True`
        // spanning [T, J] is self-contained: nothing after its loop-back J loops
        // back *into* it. For a conditional `while COND:` the continue targets the
        // condition top (T) while the true loop-back sits further down and jumps
        // into the body (an offset in (T, J]); that later back-edge means [T, J]
        // is only a fragment of a larger loop, so T is not a `while True` top.
        // A real `while True` [T, J] is self-contained: no back-edge from after J
        // loops back *into* it. A conditional `while COND:` re-tests at the loop
        // bottom (the true loop-back) and a `continue` targets the condition top —
        // both make T look like a `while True` top unless we also count those
        // back-edges. Cover every loop-back shape across 3.10-3.13:
        //   unconditional JUMP_ABSOLUTE (3.10) / JUMP_BACKWARD (3.11+),
        //   conditional POP_JUMP_BACKWARD_IF_* (3.11),
        //   conditional POP_JUMP_IF_* (3.10, absolute — can point backward).
        {
            const instrs2 = this.code.Instructions || [];
            for (let i = 0; i < instrs2.length; i++) {
                const ins = instrs2[i];
                if (!ins || ins.Offset <= J.Offset) continue;
                const t2 = this.resolveBackEdgeTarget(ins, i);
                if (t2 !== null && t2 > T && t2 <= J.Offset) return; // back-edge into our body
            }
        }

        // Conditional whiles: the body-start is immediately preceded by the entry
        // guard (a pop-jump whose target is at/after the loop-back = the exit). If
        // so, this is a guarded loop handled by the conditional-while path.
        const instrs = this.code.Instructions || [];
        let p = this.code.CurrentInstructionIndex - 1;
        while (p >= 0 && instrs[p] &&
               (instrs[p].OpCodeID === this.OpCodes.CACHE ||
                instrs[p].OpCodeID === this.OpCodes.NOT_TAKEN ||
                instrs[p].OpCodeID === this.OpCodes.INSTRUMENTED_NOT_TAKEN_A)) p--;
        const prev = p >= 0 ? instrs[p] : null;
        if (prev && this.isForwardExitGuard(prev, p, J.Offset)) return;

        // Top-tested `while COND` (3.9/3.14 single-condition shape): the loop-back
        // targets the CONDITION top T, and the guard `POP_JUMP -> exit` sits just
        // AFTER the condition — so the entry-guard-before-T check above misses it.
        // Scan forward from T: if a pop-jump whose target is past the loop-back
        // (the loop exit) appears BEFORE any body side effect, the condition is
        // evaluated first — a guarded conditional while, not `while True`. (In
        // `while True`, the body executes first, so a side effect is seen first;
        // an early `if c: break` pop-jumps to the loop-back, not past it.)
        {
            const sideEffect = new Set([
                ...
                this.OpCodes.STORE_SUBSCR, this.OpCodes.STORE_SUBSCR_A,
                this.OpCodes.RETURN_VALUE, this.OpCodes.RETURN_VALUE_A, this.OpCodes.RETURN_CONST_A,
                this.OpCodes.RAISE_VARARGS_A, this.OpCodes.YIELD_VALUE, this.OpCodes.YIELD_VALUE_A,
                this.OpCodes.IMPORT_NAME_A, this.OpCodes.DELETE_FAST_A, this.OpCodes.DELETE_NAME_A,
                this.OpCodes.PRINT_EXPR, this.OpCodes.LIST_APPEND_A, this.OpCodes.SET_ADD_A,
                this.OpCodes.MAP_ADD_A
            ].filter(v => v !== undefined));
            const guardScanOps = [
            ...
            for (let i = this.code.CurrentInstructionIndex; i < instrs.length; i++) {
                const ins = instrs[i];
                if (!ins || ins.Offset >= J.Offset) break;
                if (sideEffect.has(ins.OpCodeID)) break; // body ran first -> while True
                if (guardScanOps.includes(ins.OpCodeID) && this.isForwardExitGuard(ins, i, J.Offset + 1)) {
                    return; // guard to loop exit before any side effect -> conditional while
                }
            }
        }

        const wblk = new AST.ASTCondBlock(BT.While, T, J.Offset, null, false);
        wblk.line = cur.LineNo;
        wblk.init();
        this.blocks.push(wblk);
        this.curBlock = wblk;
        // The trailing backward jump is the implicit loop-back; skip it so it is
        // not processed as a statement after the While closes.
        (this.chainDeadRegions ||= []).push({ from: J.Offset, to: J.Offset + (J.Size || 2) });
    }

    // True if `instr` is a forward conditional pop-jump whose resolved target is
    // at or past `exitOffset` (i.e. an entry guard that can skip the loop body).
    isForwardExitGuard(instr, idx, exitOffset) {
        const guardOps = [
            this.OpCodes.POP_JUMP_IF_FALSE_A, this.OpCodes.POP_JUMP_IF_TRUE_A,
            this.OpCodes.POP_JUMP_FORWARD_IF_FALSE_A, this.OpCodes.POP_JUMP_FORWARD_IF_TRUE_A,
            this.OpCodes.POP_JUMP_IF_NONE_A, this.OpCodes.POP_JUMP_IF_NOT_NONE_A,
        ...
        if (!guardOps.includes(instr.OpCodeID)) return false;
        const reader = this.object.Reader;
        const instrs = this.code.Instructions || [];
        let arg = instr.Argument || 0;
        if (reader.versionCompare(3, 10) >= 0) arg *= 2;
        // Forward pop-jumps: relative on 3.11+ (base = next real offset), else
        // absolute (arg already *2 on 3.10).
        let tgt;
    ...
    statements () {
        ...
        while (this.code.HasInstructionsToProcess) {
            try {
                ...
                // Skip dead chained-compare tail-duplication regions. On 3.10-3.12
                // CPython copies the block tail into each chain false-exit; the
                // recognizer (consumeChainedCompareEpilogue) records those dead
                // [from,to) ranges. Jump over any range the PC lands inside so the
                // duplicated statement is not re-emitted.
                if (this.chainDeadRegions && this.chainDeadRegions.length > 0) {
                    const curOffset = this.code.Current?.Offset;
                    const region = this.chainDeadRegions.find(r => curOffset >= r.from && curOffset < r.to);
                    if (region && this.code.GetIndexByOffset(region.to) >= 0) {
                        this.code.GoToOffset(region.to);
                    }
                }

                // Open a `while True:` block when we reach an unguarded loop top
                // (an infinite loop; CPython emits no entry guard for it).
                this.maybeOpenInfiniteLoop();

                // 3.8/3.9 bottom-tested `while True: …; if c: break` (the final
                // if-break becomes a backward conditional jump; no entry guard).
                this.maybeOpenBottomTestedLoop();
