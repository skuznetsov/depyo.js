...
class PycDecompiler {
    ...
    static Decompile (obj) {
        ...
        if (funcBody.toString().length == 0 || funcBody.toString().match(/^\s*return\s*$/)) {
            return "pass";
        }
            ...
            try {
                ...
                switch (code.Current.OpCodeID)
                {
                    ...
                    case OpCodes.JUMP_ABSOLUTE:
                        // TODO: Check if it called in scope of the SETUP_LOOP
                        break;
                    ...
                    case OpCodes.SETUP_LOOP:
                        ...
                        forIter = code.GetOpCodeByID(OpCodes.FOR_ITER, -1, jumpTarget);
                        inclusiveLoopOffset = code.GetOffsetByOpCode(OpCodes.SETUP_LOOP, -1, jumpTarget);
                        if (inclusiveLoopOffset == jumpTarget) {
                            inclusiveLoopOffset = -1;
                        }
                        forOffset = forIter?.Offset || -1;
                        
                        if (forOffset > 0) {
                            jumpOffset = forIter.JumpTarget;
                            if (forIter.Prev?.OpCodeID == OpCodes.GET_ITER) {
                                getIterOffset = forIter.Prev.Offset;
                            }
                        } else {
    ...
    static MakeFunction(obj, code, DataStack, result) {
        ...
        /// Original name is more correct so we don't need it
    ...
    static DecompileIfExpression(obj, code, DataStack, parentEndOffset, result) {
        ...
        let isIfNotExp = code.Current.OpCodeID == OpCodes.POP_JUMP_IF_TRUE && !jumpOpcode.InstructionName.startsWith("POP_JUMP_IF_");
        let endOffset = jumpOpcode.OpCodeID == OpCodes.JUMP_FORWARD ? jumpOpcode.JumpTarget : parentEndOffset;
        ...
        if ([OpCodes.JUMP_FORWARD, OpCodes.JUMP_ABSOLUTE].includes(code.Current.OpCodeID)) {
            if (code.Current.Offset < elseOffset && code.Current.JumpTarget > code.Current.Offset) {
    ...
    static ExtractTestCondition(obj, code, DataStack, endOffset) {
        ...
        let ifOffests = PycDecompiler.FindIfCodesWithTheSameTarget(obj, code, code.Current.Offset, code.LastOffset);
        if ( ifOffests.length > 0 && code.Current.Offset < jumpOffset) {
            ...
            for (let ifOffset of ifOffests) {
                ...
                ifTest += op + temp;
                code.GoNext();
                jumpOffset = Math.max(jumpOffset, code.Current.JumpTarget);
            }
        }
    ...
    static DecompileComprehension(obj, code, DataStack, endOffset){
        let
            forOffset = -1,
            innerForOffset = -1,
            innerGetIterOffset = -1,
            getIterOffset = -1,
            jumpOffset = -1,
            nextOpCode = null,
            comp_for = "",
            comprehensionType = ComprehensionType.LISTCOMP;
        ...
        if (getIterOffset == -1 || getIterOffset > forOffset) {
        ...
        // Identifying if this is not inner loop or comprehension
        if (forOffset > 0) {
            innerForOffset = code.GetOffsetByOpCode(OpCodes.FOR_ITER, forOffset + 3, endOffset);
            innerGetIterOffset = code.GetOffsetByOpCode(OpCodes.GET_ITER, forOffset + 3, endOffset);
        }

        if ( innerForOffset == -1 && innerGetIterOffset > 0) {
            nextOpCode = code.PeekInstructionAtOffset(innerGetIterOffset + 1);
            if (nextOpCode.OpCodeID == OpCodes.CALL_FUNCTION && nextOpCode.Argument == 1) {
                comprehensionType = ComprehensionType.GENEXPR;
            }
        }

        let ifOffset = code.GetOffsetByOpCode(OpCodes.POP_JUMP_IF_FALSE, -1, innerForOffset > 0 ? innerForOffset : endOffset);
        let ifExpr = "";
        ...
        if (setAddOffset > listAppendOffset && setAddOffset > mapAddOffset) {
            comprehensionType = ComprehensionType.SETCOMP;
        } else if (mapAddOffset > listAppendOffset && mapAddOffset > setAddOffset) {
            comprehensionType = ComprehensionType.DICTCOMP;
        }
        
        if (comprehensionType == ComprehensionType.DICTCOMP) {
            lbracket = '{';
            rbracket = '}';
            ...
            let skip = 1;
            if (comprehensionType == ComprehensionType.LISTCOMP) {
                lbracket = '[';
                rbracket = ']';
                skip = 2;
            } else if (comprehensionType == ComprehensionType.SETCOMP) {
                lbracket = '{';
                rbracket = '}';
            } else if (comprehensionType == ComprehensionType.GENEXPR) {
                lbracket = '';
                rbracket = '';
            }
                ...
                switch (code.Current.OpCodeID) {
                    ...
                    case OpCodes.GET_ITER:
                            ...
                            (DataStack.length >= 1 && code.Next.OpCodeID == OpCodes.CALL_FUNCTION && code.Next.Argument == 1))
                        {
                            ...
                            if (co.SourceCode instanceof PycResult) {
                                source = co.SourceCode.Result[0].replace(".0", iter );
                            } else {
                                source = co.SourceCode.replace(".0", iter );
                            }
                    ...
                    case OpCodes.BUILD_MAP:
                        {
                            ...
                            if (code.Current.Argument == 0 && (![/* OpCodes.LOAD_FAST,*/ OpCodes.GET_ITER].includes(code.Next.OpCodeID))) {
                                DataStack.push("{}");
                            } else if (code.Current.Argument == 0) {
                                PycDecompiler.DecompileComprehension(obj, code, DataStack, endOffset);
                            } else {
                    ...
                    case OpCodes.MAKE_FUNCTION:
                        ...
                        let func = PycDecompiler.MakeFunction(obj, code, DataStack, functionResult);
                        if (!functionResult.HasResult) {
                            break;
                        }
                    ...
                    case OpCodes.BUILD_SLICE:
                        ...
                        break;
                    case OpCodes.MAKE_CLOSURE:
                        let closureResult = new PycResult();
                        PycDecompiler.MakeFunction(obj, code, DataStack, closureResult);
                        if (closureResult.HasResult) {
                            closureResult.DoNotIndent = true;
