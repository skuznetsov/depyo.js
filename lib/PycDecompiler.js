...
class PycDecompiler {
    ...
    static DecompileIfExpression(obj, code, DataStack, result) {
        let elseOffset = code.Current.JumpTarget;
        let [_, dominatorOp] = code.FindEndOfBlock(code.Current.JumpTarget);
        let isContinueOperator = elseOffset < code.Current.Offset && dominatorOp?.OpCodeID == OpCodes.SETUP_LOOP;
        ...
        let endOffset = (jumpOpcode.OpCodeID == OpCodes.JUMP_FORWARD && jumpOpcode.Argument > 0) ? jumpOpcode.JumpTarget : null;
        ...
        if (isContinueOperator) {
            ifBody = new PycResult("continue");
        } else {
        ...
        if (testLine == bodyLine && bodyLine == elseLine) {
    ...
    static ExtractTestCondition(obj, code, DataStack, endOffset) {
        let ifTest = "";
        let jumpOffset = code.FindEndOfBlock(endOffset);
                        ...
                        if (prevOpCode.OpCodeID == OpCodes.LOAD_FAST && prevOpCode.Argument == 0) {
                            if (["<genexpr>", "<dictcomp>", "<setcomp>"].includes(obj.Name)) {
                                ...
                                if (ifOffests.length > 0) {
                                    [ifExpr] = PycDecompiler.ExtractTestCondition(obj, code, DataStack, ifOffests.length > 0 ? ifOffests[ifOffests.length - 1] : jumpOffset);
                                }
                                yieldExpr = "";
                                ...
                                if (ifExpr?.toString().startsWith("yield ")) {
                                    ...
                                    yieldExpr = PycDecompiler.Expr(obj, code, DataStack, endOffset);
                                }
