        ...
        if (!this._withExceptRanges) {
            ...
            for (const e of entries) {
                if (e.target) {
                    ...
                    // Check for WITH_EXCEPT_START directly, or PUSH_EXC_INFO followed by WITH_EXCEPT_START
                    ...
                    if (targetInstr && (
                        ...
                        // In Python 3.11+, WITH_EXCEPT_START comes after PUSH_EXC_INFO
                    ...
                    if (isWithExceptHandler) {
                        // Mark all offsets in this exception handler as belonging to with-statement cleanup
                        // The handler typically spans ~50 bytes from target
                        for (let o = e.target; o < e.target + 50; o += 2) {
                            this._withExceptRanges.add(o);
                        }
                        if (g_cliArgs?.debug) {
                            console.log(`[EnsureExcBlocks] Built WITH handler range from ${e.target} to ${e.target + 50}`);
                        }
                    }
                }
            }
    ...
    statements () {
        ...
        while (this.code.HasInstructionsToProcess) {
            try {
                ...
                // Skip instructions within WITH exception handler ranges
                // These are the PUSH_EXC_INFO, WITH_EXCEPT_START, POP_JUMP_IF_TRUE, etc. instructions
                if (this._withExceptRanges && this._withExceptRanges.has(this.code.Current.Offset)) {
                    if (g_cliArgs?.debug) {
                        console.log(`[WithExceptSkip] Skipping ${this.code.Current.InstructionName} at offset ${this.code.Current.Offset} (within WITH exception handler)`);
                    }
                    continue;
                }
                ...
                // Pattern: LOAD_CONST None, LOAD_CONST None, LOAD_CONST None, CALL 2, POP_TOP
                if (closedWithBlock) {
                    ...
                    if (isLoadingNone) {
                        // Skip until we're past the cleanup region
                        // The cleanup is: LOAD_CONST None x3, CALL 2, POP_TOP (~16-18 bytes)
                        // Look for next PUSH_NULL or BEFORE_WITH to find the end of cleanup
                        let skipCount = 0;
                        while (this.code.HasInstructionsToProcess && skipCount < 20) {
                            const instr = this.code.Current;
                            // Stop at BEFORE_WITH, PUSH_NULL (start of next statement), or RETURN
