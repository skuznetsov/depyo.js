...
// const StackHistory = require('./stack_history');

let OpCodes = null;

Array.prototype.top = function ArrayTop () {
...
class PycDecompiler {
    ...
    static CheckIfExpr(DataStack, curBlock)
    {
        ...
        curBlock.removeLast();
        DataStack.push(new AST.ASTTernary(null, if_block, if_expr, else_expr));
    }
    ...
    static Stmts (obj, code, DataStack, endOffset = 0) {
        ...
        let blocks = [];
        // let stackHistory = new StackHistory();
        let unpack = 0;
        let else_pop = false;
        let variable_annotations = false;
        let need_try = false;
        let defBlock = new AST.ASTBlock(null, AST.ASTBlock.BlockType.Main);
        defBlock.init();
        ...
        while (code.HasInstructionsToProcess) {                
            try {                
                ...
                if (need_try && code.Current.OpCodeID != OpCodes.SETUP_EXCEPT_A) {
                    ...
                    // stackHistory.push(DataStack);
                    let tryBlock = new AST.ASTBlock(null, AST.ASTBlock.BlockType.Try, code.Current.Offset, curBlock.end, true);
                ...
                switch (code.Current.OpCodeID)
                {
                    case OpCodes.BINARY_OP_A:
                    {
                        ...
                        let node = new AST.ASTBinary(null, lVal, rVal,op);
                    ...
                    case OpCodes.BINARY_SUBSCR:
                    {
                        ...
                        let node = new AST.ASTSubscr(null, src, subscr);
                        node.line = code.Current.LineNo;
                        DataStack.push(node);
                    }
                    break;
                    ...
                    {
                        let keywordNode = new AST.ASTKeyword(null, AST.ASTKeyword.Word.Break);
                        keywordNode.line = code.Current.LineNo;
                        DataStack.push(keywordNode);
                    }
                    break;
                    case OpCodes.BUILD_CLASS:
                    {
                        ...
                        let classNode = new AST.ASTClass(null, classCode, bases, name);
                    ...
                    case OpCodes.BUILD_FUNCTION:
                    {
                        ...
                        let functionNode = new AST.ASTFunction(null, functionCode);
                        functionNode.line = code.Current.LineNo;
                    ...
                    case OpCodes.BUILD_LIST_A:
                    {
                        ...
                        let listNode = new AST.ASTList(null, values);
                    ...
                    case OpCodes.BUILD_SET_A:
                    {
                        ...
                        let listNode = new AST.ASTSet(null, values);
                        DataStack.push(listNode);
                    }
                    break;
                    ...
                    case OpCodes.BUILD_MAP_A:
                    {
                        if (obj.Reader.versionCompare(3, 5) >= 0) {
                            let mapNode = new AST.ASTMap(null);
                    ...
                    case OpCodes.BUILD_CONST_KEY_MAP_A:
                    {
                        ...
                        let mapNode = new AST.ASTConstMap(null, keys, values);
                        mapNode.line = code.Current.LineNo;
                        DataStack.push(mapNode);
                    }
                    break;
                    ...
                    case OpCodes.BUILD_SLICE_A:
                    {
                        if (code.Current.Argument == 2) {
                            ...
                            let mapNode = new AST.ASTSlice(null, sliceOp, start || new AST.ASTNone(null), end || new AST.ASTNone(null));
                            mapNode.line = code.Current.LineNo;
                            DataStack.push(mapNode);
                            ...
                            let lhs = new AST.ASTSlice(null, sliceOp, start || new AST.ASTNone(null), end || new AST.ASTNone(null));
                            ...
                            let sliceNode = new AST.ASTSlice(null, sliceOp, start || new AST.ASTNone(null), end || new AST.ASTNone(null));
                    ...
                    case OpCodes.BUILD_STRING_A:
                    {
                        ...
                        let stringNode = new AST.ASTJoinedStr(null, values);
                    ...
                    case OpCodes.BUILD_TUPLE_A:
                    {
                        ...
                        let tupleNode = new AST.ASTTuple(null, values);
                    ...
                    case OpCodes.KW_NAMES_A:
                    {
                        ...
                        let astNode = new AST.ASTKwNamesMap(null);
                    ...
                    case OpCodes.INSTRUMENTED_CALL_A:
                    {
                        ...
                        if (loadBuildClassFound) {
                            ...
                            if (loadbuild instanceof AST.ASTLoadBuildClass) {
                                let callNode = new AST.ASTCall(null, functionNode, pparamList, kwparamList);
                                callNode.line = code.Current.LineNo;
                                let classNode = new AST.ASTClass(null, call, new AST.ASTTuple(null, bases), name);
                                classNode.line = code.Current.LineNo;
                                DataStack.push(classNode);
                                // stackHistory.pop();
                                break;
                        ...
                        */
                        ...
                        for (let idx = 0; idx < pparams; idx++) {
                            ...
                            if (param instanceof AST.ASTFunction) {
                                ...
                                if (function_name == "<lambda>") {
                                    ...
                                    // Decorator used
                                    let decorNameNode = new AST.ASTName(null, function_name);
                                    let storeNode = new AST.ASTStore(null, param, decorNameNode);
                        ...
                        let callNode = new AST.ASTCall( null, func, pparamList, kwparamList);
                        callNode.line = code.Current.LineNo;
                    ...
                    case OpCodes.CALL_METHOD_A:
                    {
                        ...
                        let func = DataStack.pop();
                        let callNode = new AST.ASTCall (null, func, pparamList, []);
                        callNode.line = code.Current.LineNo;
                        DataStack.push(callNode);
                    }
                    break;
                    ...
                    {
                        let node = new AST.ASTKeyword (null, AST.ASTKeyword.Word.Continue);
                        node.line = code.Current.LineNo;
                    ...
                    case OpCodes.COMPARE_OP_A:
                    {
                        ...
                        let node = new AST.ASTCompare (null, left, right, arg);
                        node.line = code.Current.LineNo;
                        DataStack.push(node);
                    }
                    break;
                    case OpCodes.CONTAINS_OP_A:
                    {
                        ...
                        let node = new AST.ASTCompare (null, left, right, code.Current.Argument ? AST.ASTCompare.CompareOp.NotIn : AST.ASTCompare.CompareOp.In);
                        node.line = code.Current.LineNo;
                        DataStack.push(node);
                    ...
                    {
                        let name = DataStack.pop();
                        let node = new AST.ASTDelete(new AST.ASTBinary(name, new AST.ASTName(null, code.Current.Name), AST.ASTBinary.BinOp.Attr));
                        node.line = code.Current.LineNo;
                        curBlock.nodes.push(node);
                    }
                    break;
                    ...
                    case OpCodes.DELETE_NAME_A:
                    {
                        ...
                        if (varname.length >= 2 && varname[0] == '_'
                                && varname[1] == '[') {
                            /* Don't show deletes that are a result of list comps. */
                            break;
                        }
        
                        let node = new AST.ASTDelete(null, new AST.ASTName(null, varname));
                        node.line = code.Current.LineNo;
                        curBlock.nodes.push(node);
                    }
                    break;
                    case OpCodes.DELETE_FAST_A:
                    {
                        ...
                        if (obj.Reader.versionCompare(1, 3) < 0)
                            nameNode = new AST.ASTName(null, code.Current.Name);
                        else
                            nameNode = new AST.ASTName(null, code.Current.LocalName);
        
                        if (nameNode.name[0] == '_' && nameNode.name[1] == '[') {
                            /* Don't show deletes that are a result of list comps. */
                            break;
                        }
        
                        let node = new AST.ASTDelete(null, nameNode);
                        node.line = code.Current.LineNo;
                        curBlock.nodes.push(node);
                    }
                    break;
                    ...
                    {
                        let name = DataStack.pop();
                        let node = new AST.ASTDelete(null, new AST.ASTSubscr(null, name, new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice0)));
                        node.line = code.Current.LineNo;
                        curBlock.nodes.push(node);
                    }
                    break;
                    case OpCodes.DELETE_SLICE_1:
                    {
                        ...
                        let node = new AST.ASTDelete(null, new AST.ASTSubscr(null, name, new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice1, upper)));
                        node.line = code.Current.LineNo;
                        curBlock.nodes.push(node);
                    }
                    break;
                    case OpCodes.DELETE_SLICE_2:
                    {
                        ...
                        let node = new AST.ASTDelete(null, new AST.ASTSubscr(null, name, new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice2, null, lower)));
                        node.line = code.Current.LineNo;
                        curBlock.nodes.push(node);
                    }
                    break;
                    case OpCodes.DELETE_SLICE_3:
                    {
                        ...
                        let node = new AST.ASTDelete(null, new AST.ASTSubscr(null, name, new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice3, upper, lower)));
                        node.line = code.Current.LineNo;
                        curBlock.nodes.push(node);
                    }
                    break;
                    case OpCodes.DELETE_SUBSCR:
                    {
                        ...
                        let node = new AST.ASTDelete(null, new AST.ASTSubscr(null, name, key));
                        node.line = code.Current.LineNo;
                        curBlock.nodes.push(node);
                    }
                    break;
                    case OpCodes.DUP_TOP:
                    {
                        if (DataStack.top() == null) {
                            ...
                            DataStack.push(DataStack.top());
                            let node = new AST.ASTChainStore (null, [], DataStack.top());
                            DataStack.push(node);
                        }
                    }
                    break;
                        ...
                        if (curBlock.blockType == AST.ASTBlock.BlockType.Finally) {
                            ...
                            if (!isUninitAsyncFor) {
                                ...
                                if (curBlock.end != code.Next.Offset || curBlock.hasFinally) {
                                    let elseblk = new AST.ASTBlock(null, AST.ASTBlock.BlockType.Else, code.Current.Offset, prev.end);
                                    elseblk.init();
                                    blocks.push(elseblk);
                                    curBlock = blocks.top();
                                }
                    ...
                    case OpCodes.EXEC_STMT:
                    {
                        ...
                        let node = new AST.ASTExec(null, stmt, glob, loc);
                        node.line = code.Current.LineNo;
                    ...
                    case OpCodes.INSTRUMENTED_FOR_ITER_A:
                    {
                        ...
                        let forblk = new AST.ASTIterBlock(null, AST.ASTBlock.BlockType.For, code.Current.Offset, end, iter);
                    ...
                    case OpCodes.FOR_LOOP_A:
                    {
                        ...
                        let forblk = new AST.ASTIterBlock(null, AST.ASTBlock.BlockType.For, code.Current.Offset, top.end, iter);
                        forblk.line = code.Current.LineNo;
                        forblk.comprehension = comprehension;
                        blocks.push(forblk);
                        curBlock = blocks.top();
                    ...
                    case OpCodes.GET_AITER:
                    {
                        ...
                        if (top.blockType == AST.ASTBlock.BlockType.While) {
                            blocks.pop();
                            let forblk = new AST.ASTIterBlock(null, AST.ASTBlock.BlockType.AsyncFor, top.start, top.end, iter);
                            forblk.line = code.Current.LineNo;
                            blocks.push(forblk);
                            curBlock = blocks.top();
                            DataStack.push(null);
                    ...
                    case OpCodes.FORMAT_VALUE_A:
                    {
                        ...
                        switch (conversion_flag) {
                            ...
                            case AST.ASTFormattedValue.ConversionFlag.ASCII:
                            {
                                ...
                                let node = new AST.ASTFormattedValue (null, val, conversion_flag, null);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);
                            }
                            break;
                            case AST.ASTFormattedValue.ConversionFlag.FmtSpec:
                            {
                                ...
                                let val = DataStack.pop();
                                let node = new AST.ASTFormattedValue (null, val, conversion_flag, format_spec);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);
                            }
                            break;
                    ...
                    case OpCodes.GET_AWAITABLE:
                        {
                            ...
                            let node = new AST.ASTAwaitable (null, object);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                    }
                        break;
                    ...
                    case OpCodes.IMPORT_NAME_A:
                        if (obj.Reader.versionCompare(2, 0) < 0) {
                            let node = new AST.ASTImport(null, new AST.ASTName(null, code.Current.Name), null);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        } else {
                            ...
                            if (obj.Reader.versionCompare(2, 5) >= 0) {
                                ...
                                let importLevel = +importLevelNode.object;
                                if (importLevel > 0) {
                                    dots = Buffer.alloc(importLevel, '.').toString('ascii');
                                }
                            }

                            let node = new AST.ASTImport (null, new AST.ASTName(null, dots + code.Current.Name), fromlist);
                            node.line = code.Current.LineNo;

                            if (code.Next.OpCodeID == OpCodes.IMPORT_STAR) {
                                node.add_store(new AST.ASTStore(null, new AST.ASTName(null, "*"), null))
                                ...
                                code.extractImportNames(fromlist.object, (name, alias) => {
                                    node.add_store(new AST.ASTStore(null, new AST.ASTName(null, name), new AST.ASTName(null, alias)));
                                });
                            } else if (!fromlist) {
                                let aliasNode = code.GetOpCodeByName("STORE_*");
                                node.alias = new AST.ASTName(null, aliasNode.Label);
                    ...
                    case OpCodes.IS_OP_A:
                    {
                        ...
                        let node = new AST.ASTCompare (null, left, right, code.Current.Argument ? AST.ASTCompare.CompareOp.IsNot : AST.ASTCompare.CompareOp.Is);
                        node.line = code.Current.LineNo;
                        curBlock.append(node);
                    }
                    break;
                    ...
                    case OpCodes.INSTRUMENTED_POP_JUMP_IF_TRUE_A:
                    {
                        ...
                        if (code.Current.OpCodeID == OpCodes.POP_JUMP_IF_FALSE_A ||
                            code.Current.OpCodeID == OpCodes.POP_JUMP_IF_TRUE_A ||
                            code.Current.OpCodeID == OpCodes.POP_JUMP_FORWARD_IF_FALSE_A || 
                            code.Current.OpCodeID == OpCodes.POP_JUMP_FORWARD_IF_TRUE_A || 
                            code.Current.OpCodeID == OpCodes.INSTRUMENTED_POP_JUMP_IF_FALSE_A || 
                            code.Current.OpCodeID == OpCodes.INSTRUMENTED_POP_JUMP_IF_TRUE_A) {
                        ...
                        if (code.Current.OpCodeID == OpCodes.JUMP_IF_FALSE_OR_POP_A || code.Current.OpCodeID == OpCodes.JUMP_IF_TRUE_OR_POP_A) {
                        ...
                        let neg =  code.Current.OpCodeID == OpCodes.JUMP_IF_TRUE_A
                                || code.Current.OpCodeID == OpCodes.JUMP_IF_TRUE_OR_POP_A
                                || code.Current.OpCodeID == OpCodes.POP_JUMP_IF_TRUE_A
                                || code.Current.OpCodeID == OpCodes.POP_JUMP_FORWARD_IF_TRUE_A
                                || code.Current.OpCodeID == OpCodes.INSTRUMENTED_POP_JUMP_IF_TRUE_A;
                        ...
                        if (obj.Reader.versionCompare(3, 10) >= 0)
                            offs *= 2; // // BPO-27129
                        if (obj.Reader.versionCompare(3, 12) >= 0
                            || code.Current.OpCodeID == OpCodes.JUMP_IF_FALSE_A
                            || code.Current.OpCodeID == OpCodes.JUMP_IF_TRUE_A
                            || code.Current.OpCodeID == OpCodes.POP_JUMP_FORWARD_IF_TRUE_A
                            || code.Current.OpCodeID == OpCodes.POP_JUMP_FORWARD_IF_FALSE_A) {
                        ...
                        if (cond instanceof AST.ASTCompare
                            ...
                            ifblk = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.Except, code.Current.Offset, offs, cond.right, false);
                            ...
                            blocks.pop();
                            // DataStack = stackHistory.pop();
                            ifblk = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.Elif, startOffset, offs, cond, neg);
                            ...
                            // if (curBlock.blockType == AST.ASTBlock.BlockType.While) {
                            //     stackHistory.pop();
                            // } else {
                            //     let s_top = stackHistory.top();
                            //     stackHistory.pop();
                            //     stackHistory.pop();
                            //     stackHistory.push(s_top);
                            // }
        
                            if (curBlock.end == offs
                                    || (curBlock.end == code.Current.Offset && !top.negative)) {
                                /* if blah and blah */
                                newcond = new AST.ASTBinary(null, cond1, cond, AST.ASTBinary.BinOp.LogicalAnd);
                            } else {
                                /* if <condition 1> or <condition 2> */
                                newcond = new AST.ASTBinary(null, cond1, cond, AST.ASTBinary.BinOp.LogicalOr);
                            }
                            newcond.line = code.Current.LineNo;
                            ifblk = new AST.ASTCondBlock(null, top.blockType, code.Current.Offset, offs, newcond, neg);
                            ...
                            // let oldDataStack = stackHistory.pop();
                            // let listNode = oldDataStack.top();
                            // if (listNode && listNode instanceof AST.ASTList && listNode.values.length == 0) {
                            //     // This is a comprehension. Saving it to DataStack
                            //     let a = 0;
                            //     // DataStack.push(curBlock);
                            // }
                            break;
                        } else {
                            /* Plain old if statement */
                            ifblk = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.If, code.Current.Offset, offs, cond, neg);
                        }
                    ...
                    case OpCodes.JUMP_ABSOLUTE_A:
                        {
                            ...
                            if (offs < code.Next.Offset) {
                                if (curBlock.blockType == AST.ASTBlock.BlockType.For) {
                                    ...
                                    // Not necessarily. It can be an optimised bytecode, when the block at the end of previous block
                                    // TODO: Check parent blocks if they cover this range and if current address is equal to the end of the block.
                                    curBlock.append(new AST.ASTKeyword(null, AST.ASTKeyword.Word.Continue));
                                }
                            ...
                            if (curBlock.blockType == AST.ASTBlock.BlockType.Container) {
                                ...
                                if (cont.hasExcept && code.Next.Offset < cont.except) {
                                    let except = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.Except, code.Current.Offset, code.Current.JumpTarget, null, false);
                                    except.init();
                                    blocks.push(except);
                                    curBlock = blocks.top();
                                }
                            ...
                            if (blocks.length > 1) {
                                do {
                                    ...
                                    if ([
                                        ...
                                        // }
                                        let top = blocks.top();
                                        let next = new AST.ASTBlock(null, AST.ASTBlock.BlockType.Else, code.Current.Offset, top.end);
                                        ...
                                        // }
                                        let top = blocks.top();
                                        let next = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.Except, top.start, top.end, null, false);
                                        next.init();
                    ...
                    case OpCodes.INSTRUMENTED_JUMP_FORWARD_A:
                        {
                            ...
                            if (curBlock.blockType == AST.ASTBlock.BlockType.Container) {
                                ...
                                if (cont.hasExcept) {
                                    ...
                                    let except = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.Except, code.Current.Offset, curBlock.end, null, false);
                                    except.init();
                                    blocks.push(except);
                                    curBlock = blocks.top();
                                }
                            ...
                            // if (!stackHistory.empty()) {
                            //     if (DataStack.empty()) // if it's part of if-expression, TOS at the moment is the result of "if" part
                            //         DataStack = stackHistory.top();
                            //     stackHistory.pop();
                            // }
                            ...
                            if (blocks.length > 1) {
                                do {
                                    ...
                                    if (prev.blockType == AST.ASTBlock.BlockType.If
                                        ...
                                        //     stackHistory.push(DataStack);
                                        // }
                                        let next = new AST.ASTBlock(null, AST.ASTBlock.BlockType.Else, code.Current.Offset, code.Next.Offset + offs);
                                        if (prev.inited == AST.ASTCondBlock.InitCondition.PrePopped) {
                                            next.init(AST.ASTCondBlock.InitCondition.PrePopped);
                                        }
                                        ...
                                        //     stackHistory.push(DataStack);
                                        // }
                                        let next = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.Except, code.Current.Offset, code.Next.Offset + offs, null, false);
                                        next.init();
                                        ...
                                        if (blocks.top().blockType == AST.ASTBlock.BlockType.Container) {
                                            ...
                                            if (cont.hasExcept) {
                                                ...
                                                let except = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.Except, prev.end, code.Next.Offset + offs, null, false);
                                                except.init();
                                                blocks.push(except);
                                            }
                                        } else {
                        ...
                        case OpCodes.LIST_APPEND_A:
                        {
                            ...
                            if (curBlock.blockType == AST.ASTBlock.BlockType.For && curBlock.comprehension) {
                                DataStack.pop();
                                let node = new AST.ASTComprehension (null, value);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);
                            } else {
                                let node = new AST.ASTSubscr (null, list, value);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);
                            }
                        }
                        ...
                        case OpCodes.SET_UPDATE_A:
                        {
                            ...
                            for (let value of obj.values) {
                                result.push(new AST.ASTObject (null, value));
                            }
            
                            let node = new AST.ASTSet (null, result);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        break;
                        case OpCodes.LIST_EXTEND_A:
                        {
                            ...
                            let node = new AST.ASTList (null, result);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        break;
                        case OpCodes.LOAD_ATTR_A:
                        {
                            ...
                            if (!(name instanceof AST.ASTImport)) {
                                ...
                                let node = new AST.ASTBinary(null, name, new AST.ASTName(null, code.Current.Name), AST.ASTBinary.BinOp.Attr);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);
                            }
                        }
                        ...
                        {
                            let node = new AST.ASTLoadBuildClass (null, new PycObject());
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        break;
                        ...
                        {
                            let constantObject = new AST.ASTObject(null, code.Current.ConstantObject);
                        ...
                        {
                            let node = new AST.ASTName (null, code.Current.FreeName);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        break;
                        case OpCodes.LOAD_FAST_A:
                        {
                            let node = null;
                            if (obj.Reader.versionCompare(1, 3) < 0) {
                                node = new AST.ASTName (null, code.Current.Name);
                            } else {
                                node = new AST.ASTName (null, code.Current.LocalName);
                            }
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        ...
                        case OpCodes.LOAD_GLOBAL_A:
                        {
                            ...
                            let node = new AST.ASTName (null, code.Current.Name);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        break;
                        ...
                        {
                            let node = new AST.ASTLocals (null);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        break;
                        ...
                        case OpCodes.POP_BLOCK:
                        {
                            ...
                            //         AST.ASTBlock.BlockType.Try,
                            //         AST.ASTBlock.BlockType.Except,
                            //         AST.ASTBlock.BlockType.Finally
                            //     ].includes(curBlock.blockType)) {
                            //     if (!stackHistory.empty()) {
                            //         DataStack = stackHistory.pop();
                            //     } else {
                            //         console.error("Warning: Stack history is empty, something wrong might have happened\n");
                            //     }
                            // }
                            let tmp = blocks.pop();
                            ...
                            if (tmp.blockType == AST.ASTBlock.BlockType.For && tmp.end >= code.Next.Offset) {
                                // stackHistory.push(DataStack);
            
                                let blkElse = new AST.ASTBlock(null, AST.ASTBlock.BlockType.Else, code.Current.Offset, tmp.end);
                                blocks.push(blkElse);
                                curBlock = blocks.top();
                            }
                            ...
                            if (curBlock.blockType == AST.ASTBlock.BlockType.Container) {            
                                if (tmp.blockType == AST.ASTBlock.BlockType.Else && !curBlock.hasFinally) {
                                    ...
                                    let final = new AST.ASTBlock(null, AST.ASTBlock.BlockType.Finally, tmp.start, tmp.end, true);
                                    blocks.push(final);
                                    curBlock = blocks.top();
                                }
                            }
                        ...
                        case OpCodes.POP_TOP:
                        {
                            ...
                            if (curBlock.blockType == AST.ASTBlock.BlockType.For
                                ...
                                if (value instanceof AST.ASTCall) {
                                    ...
                                    if (!pparams.empty()) {
                                        let res = pparams[0];
                                        let node = new AST.ASTComprehension (null, res);
                                        node.line = code.Current.LineNo;
                                        DataStack.push(node);
                                    }
                                }
                        ...
                        {
                            let printNode;
                            if (curBlock.length > 0 && curBlock.nodes.top() instanceof AST.ASTPrint) {
                                printNode = curBlock.nodes.top();
                            }
                            if (printNode && printNode.stream == null && !printNode.eol) {
                                printNode.add(DataStack.pop());
                            } else {
                                let node = new AST.ASTPrint(null, DataStack.pop());
                                curBlock.append(node);
                            }
                        }
                        break;
                        case OpCodes.PRINT_ITEM_TO:
                        {
                            ...
                            if (printNode && printNode.stream == stream && !printNode.eol) {
                                ...
                                let node = new AST.ASTPrint(null, DataStack.pop(), stream);
                                curBlock.append(node);
                            }
                            stream.setProcessed();
                        }
                        ...
                        case OpCodes.PRINT_NEWLINE:
                        {
                            ...
                            if (printNode && printNode.stream == null && !printNode.eol)
                                ...
                                let node = new AST.ASTPrint(null);
                                node.line = code.Current.LineNo;
                                curBlock.append(node);
                            }
                            DataStack.pop();
                        ...
                        case OpCodes.PRINT_NEWLINE_TO:
                        {
                            ...
                            if (printNode && printNode.stream == stream && !printNode.eol) {
                                ...
                                let node = new AST.ASTPrint(null, null, stream);
                                node.line = code.Current.LineNo;
                                curBlock.append(node);
                            }
                            DataStack.pop();
                        ...
                        case OpCodes.RAISE_VARARGS_A:
                        {
                            ...
                            let node = new AST.ASTRaise(null, paramList);
                            node.line = code.Current.LineNo;
                            curBlock.append(node);
                        ...
                        {
                            let value = DataStack.pop();
                            let node = new AST.ASTReturn(null, value);
                            node.line = code.Current.LineNo;
                            curBlock.append(node);
                        ...
                        {
                            let value = new AST.ASTObject(null, code.Current.ConstantObject);
                            let node = new AST.ASTReturn(null, value);
                            node.line = code.Current.LineNo;
                            curBlock.append(node);
                        }
                        ...
                        {
                            let withBlock = new AST.ASTWithBlock(null, code.Current.Offset, code.Current.JumpTarget);
                        ...
                        case OpCodes.SETUP_EXCEPT_A:
                        {
                            if (curBlock.blockType == AST.ASTBlock.BlockType.Container) {
                                ...
                                let nextBlock = new AST.ASTContainerBlock(null, code.Current.Offset, 0, code.Current.JumpTarget);
                                blocks.push(nextBlock);
                            }
            
                            /* Store the current stack for the except/finally statement(s) */
                            // stackHistory.push(DataStack);
                            let tryBlock = new AST.ASTBlock(null, AST.ASTBlock.BlockType.Try, code.Current.Offset, code.Current.JumpTarget, true);
                            blocks.push(tryBlock);
                            curBlock = blocks.top();
                        ...
                        {
                            let nextBlock = new AST.ASTContainerBlock(null, code.Current.Offset, code.Current.JumpTarget);
                            blocks.push(nextBlock);
                            curBlock = blocks.top();
                        ...
                        {
                            let nextBlock = new AST.ASTCondBlock(null, AST.ASTBlock.BlockType.While, code.Current.Offset, code.Current.JumpTarget, null, false);
                            blocks.push(nextBlock);
                            curBlock = blocks.top();
                        }
                        break;
                        case OpCodes.SLICE_0:
                        {
                            ...
                            let sliceNode = new AST.ASTSlice (null, AST.ASTSlice.SliceOp.Slice0);
                            let node = new AST.ASTSubscr (null, name, sliceNode);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        break;
                        case OpCodes.SLICE_1:
                        {
                            ...
                            let sliceNode = new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice1, lower);
                            let node = new AST.ASTSubscr(null, name, sliceNode);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        ...
                        case OpCodes.SLICE_2:
                        {
                            ...
                            let sliceNode = new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice2, null, upper);
                            let node = new AST.ASTSubscr(null, name, sliceNode);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);    
                        }
                        ...
                        case OpCodes.SLICE_3:
                        {
                            ...
                            let sliceNode = new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice3, lower, upper);
                            let node = new AST.ASTSubscr(null, name, sliceNode);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);    
                        }
                        ...
                        case OpCodes.STORE_ATTR_A:
                        {
                            if (unpack) {
                                let name = DataStack.pop();
                                let attrNode = new AST.ASTBinary(null, name, new AST.ASTName(null, code.Current.Name), AST.ASTBinary.BinOp.Attr);
                                ...
                                if (--unpack <= 0) {
                                    ...
                                    if (seqNode instanceof AST.ASTChainStore) {
                                        ...
                                        let node = new AST.ASTStore(null, seqNode, tup);
                                        node.line = code.Current.LineNo;
                                        curBlock.append(node);
                                    }
                                }
                                ...
                                if (value instanceof AST.ASTChainStore) {
                                    ...
                                    let node = new AST.ASTStore(null, value, attrNode);
                                    node.line = code.Current.LineNo;
                                    curBlock.append(node);
                                }
                            }
                        ...
                        {
                            if (unpack) {
                                let nameNode = new AST.ASTName(null, code.Current.FreeName);
                                ...
                                if (--unpack <= 0) {
                                    ...
                                    if (seqNode instanceof AST.ASTChainStore) {
                                        ...
                                        let node = new AST.ASTStore(null, seqNode, tupleNode);
                                        node.line = code.Current.LineNo;
                                        curBlock.append(node);
                                    }
                                }
                                ...
                                if (valueNode instanceof AST.ASTChainStore) {
                                    if (valueNode) {
                                        valueNode.line = code.Current.LineNo;
                                    }
                                    PycDecompiler.append_to_chain_store(valueNode, nameNode, DataStack, curBlock);
                                } else {
                                    let node = new AST.ASTStore(null, valueNode, nameNode);
                                    node.line = code.Current.LineNo;
                                    curBlock.append(node);
                                }
                            }
                        ...
                        case OpCodes.STORE_FAST_A:
                        {
                            if (unpack) {
                                ...
                                if (
                                    curBlock.blockType == AST.ASTBlock.BlockType.Except &&
                                    code.Prev.OpCodeID == OpCodes.POP_TOP &&
                                    code.Next.OpCodeID == OpCodes.POP_TOP
                                ) {
                                    let cond = curBlock.condition;
                                    let condNode = new AST.ASTStore(null, cond, nameNode);
                                    curBlock.condition = condNode;
                                } else if (curBlock.blockType == AST.ASTBlock.BlockType.For
                                        && !curBlock.inited) {
                                ...
                                } else if (valueNode instanceof AST.ASTChainStore) {
                                    if (valueNode) {
                                        valueNode.line = code.Current.LineNo;
                                    }
                                    PycDecompiler.append_to_chain_store(valueNode, nameNode, DataStack, curBlock);
                                    ...
                                    let node = new AST.ASTStore(null, valueNode, nameNode);
                                    node.line = valueNode?.line > -1 ? valueNode?.line : code.Current.LineNo;
                                    curBlock.append(node);
                                }
                            }
                        }
                        ...
                        {
                            let nameNode = new AST.ASTName(null, code.Current.Name);
                        ...
                        case OpCodes.STORE_NAME_A:
                        {
                            if (unpack) {
                                ...
                                let nameNode = new AST.ASTName(null, varName);
                                ...
                                if (curBlock.blockType == AST.ASTBlock.BlockType.For
                                    ...
                                    let storeNode = new AST.ASTStore(null, valueNode, nameNode);
                                    storeNode.line = code.Current.LineNo;
                        ...
                        case OpCodes.STORE_SLICE_0:
                        {
                            ...
                            let valueNode = DataStack.pop();
                            let node = new AST.ASTStore(null, valueNode,
                                            new AST.ASTSubscr(null, destNode,
                                                new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice0)
                                            )
                                        );
                            node.line = code.Current.LineNo;
                            curBlock.append(node);
                        ...
                        case OpCodes.STORE_SLICE_1:
                        {
                            ...
                            let value = DataStack.pop();
                            let node = new AST.ASTStore(null, value,
                                new AST.ASTSubscr(null, dest,
                                    new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice1, upper)
                                )
                            );
                            node.line = code.Current.LineNo;
                            curBlock.append(node);
                        ...
                        case OpCodes.STORE_SLICE_2:
                        {
                            ...
                            let node = new AST.ASTStore(null, valueNode,
                                new AST.ASTSubscr(null, destNode,
                                    new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice2, null, lowerNode)));
                            node.line = code.Current.LineNo;
                            curBlock.append(node);
                        }
                        break;
                        case OpCodes.STORE_SLICE_3:
                        {
                            ...
                            let node = new AST.ASTStore(null, valueNode,
                                new AST.ASTSubscr(null, destNode,
                                    new AST.ASTSlice(null, AST.ASTSlice.SliceOp.Slice3, upperNode, lowerNode)));
                            node.line = code.Current.LineNo;
                            curBlock.append(node);
                        }
                        break;
                            ...
                            if (unpack) {
                                ...
                                if (found_annotated_var) {
                                    ...
                                    if (!curBlock.nodes.empty() && curBlock.nodes.top() instanceof AST.ASTStore) {
                                        ...
                                        curBlock.removeLast();
                                        let node = new AST.ASTStore(null, store.src,
                                            new AST.ASTAnnotatedVar(null, subscrNode, srcNode)
                                        );
                                        node.line = code.Current.LineNo;
                                        curBlock.append(node);
                                    } else {
                                        let node = new AST.ASTAnnotatedVar(null, subscrNode, srcNode);
                                        node.line = code.Current.LineNo;
                                        curBlock.append(node);
                                    }
                                } else {
                                    if (destNode instanceof AST.ASTMap) {
                                        ...
                                        srcNode.line = code.Current.LineNo;
                                        PycDecompiler.append_to_chain_store(srcNode, new AST.ASTSubscr(null, destNode, subscrNode), DataStack, curBlock);
                                    } else {
                                        let node = new AST.ASTStore(null, srcNode,
                                            new AST.ASTSubscr(null, destNode, subscrNode)
                                        );
                                        node.line = code.Current.LineNo;
                                        curBlock.append(node);
                                    }
                        ...
                        case OpCodes.UNARY_CALL:
                        {
                            ...
                            let node = new AST.ASTCall(null, funcNode, [], []);
                            node.line = code.Current.LineNo;
                            DataStack.push(node);
                        }
                        break;
                        case OpCodes.UNARY_CONVERT:
                            {
                                let name = DataStack.pop();
                                let node = new AST.ASTConvert(null, name);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);
                            }
                            break;
                        case OpCodes.UNARY_INVERT:
                            {
                                ...
                                let node = new AST.ASTUnary(null, arg, AST.ASTUnary.UnaryOp.Invert);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);    
                            }
                            break;
                        case OpCodes.UNARY_NEGATIVE:
                            {
                                let arg = DataStack.pop();
                                let node = new AST.ASTUnary(null, arg, AST.ASTUnary.UnaryOp.Negative);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);    
                            }
                            break;
                        case OpCodes.UNARY_NOT:
                            {
                                let arg = DataStack.pop();
                                let node = new AST.ASTUnary(null, arg, AST.ASTUnary.UnaryOp.Not);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);    
                            }
                            break;
                        case OpCodes.UNARY_POSITIVE:
                            {
                                let arg = DataStack.pop();
                                let node = new AST.ASTUnary(null, arg, AST.ASTUnary.UnaryOp.Positive);
                                node.line = code.Current.LineNo;
                                DataStack.push(node);    
                            }
                            break;
                        ...
                        case OpCodes.UNPACK_SEQUENCE_A:
                        {
                            ...
                            if (unpack > 0) {
                                ...
                                // E.g. [] = TOS / for [] in X
                                let tupleNode = new AST.ASTTuple(null, []);
                                if (curBlock.blockType == AST.ASTBlock.BlockType.For
                                    && !curBlock.inited) {
                                    ...
                                    let node = new AST.ASTStore(null, DataStack.pop(), tupleNode);
                                    node.line = code.Current.LineNo;
                                    curBlock.append(node);
                                }
                            }
                        ...
                        case OpCodes.YIELD_FROM:
                        {
                            ...
                            if (valueNode) {
                                valueNode.setProcessed();
                                let node = new AST.ASTReturn(null, valueNode, AST.ASTReturn.RetType.YieldFrom);
                                node.line = code.Current.LineNo;
                                curBlock.append(node);
                            }
                        }
                        ...
                        {
                            let value = DataStack.pop();
                            let node = new AST.ASTReturn(null, value, AST.ASTReturn.RetType.Yield);
                            node.line = code.Current.LineNo;
                            curBlock.append(node);
                        }
                        break;
                        ...
                        default:
                        {
                            ...
                            let node = new AST.ASTNodeList(null, defBlock.nodes);
                            return node;
                        }
                    }
                ...
                return `EXCEPTION for OpCode ${code.Current.InstructionName} (${code.Current.Argument}) at offset ${code.Current.Offset} in code object '${obj.Name}' : ${ex.message}\n\n`;
            }
        }
        // if (!stackHistory.empty()) {
        //     console.error("Warning: Stack history is not empty!\n");
        ...
        let mainNode = new AST.ASTNodeList(null, defBlock.nodes);
        return mainNode;
    }
}
