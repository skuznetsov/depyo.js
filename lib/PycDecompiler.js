...
class PycDecompiler {
    ...
    transformExceptionGroups(root) {
        ...
        this.wrapFunctionExceptionGroups(root);
    }
    ...
    rewriteExceptionGroupsInList(listNode) {
        ...
        const firstExceptIdx = nodes.findIndex(node => this.isExceptStarBlock(node));
        if (firstExceptIdx === -1) {
            return;
        }

        const tryBlock = new AST.ASTBlock(AST.ASTBlock.BlockType.Try, nodes[0]?.start || 0, 0, true);
        tryBlock.nodes.push(...nodes.slice(0, firstExceptIdx));
