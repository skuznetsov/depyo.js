...
class PycReader
{
    ...
    ParseExceptionTable(exceptTableObject) {
        /**
         * Parse Python 3.11+ exception table format
         * Format: delta-encoded varints
         * Each entry: start_delta, length, target, depth_lasti
         */
        ...
        while (pos < data.length) {
            // Start offset (delta from previous end)
            const startDelta = decodeVarint();
            const start = prevOffset + startDelta * 2;
            ...
            const target = decodeVarint();
            const targetOffset = target * 2;

            // Stack depth and lasti flag
