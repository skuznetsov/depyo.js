        ...
        try {
            ...
            switch (objectType) {
                ...
                case TypeLong:
                    ...
                    // CPython r_PyLong uses C integer division here, which floors.
                    // JS `/` is float division, so without Math.floor an even
                    // marshal-digit count (|n| >= 4) over-runs the digit loop and
                    // reads 2 extra shorts, desyncing the whole marshal stream
                    // (e.g. the `1 << 48` constant in uuid.py — issue #13).
                    let size = 1 + Math.floor((Math.abs(n) - 1) / PyLong_MARSHAL_RATIO);
                    ...
                    obj.ClassName = "Py_VeryLong";
                    obj.Value = longValue;
                    // r_long's sign carries the sign of the whole long; the digit
                    // loop above used Math.abs(n), so record it for rendering.
                    obj.IsNegative = n < 0;
                    break;
                case TypeFloat:
