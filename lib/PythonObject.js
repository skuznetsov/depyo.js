    ...
    toString()
    {
        switch(this.ClassName) {
            ...
            case "Py_VeryLong":
                ...
                // TYPE_LONG: an array of little-endian base-2^30 digits, the way
                // PycReader packs PyLong_MARSHAL_RATIO(=2) 15-bit shorts per slot.
                // Fold them into a single integer with BigInt so any width renders
                // as a valid decimal literal (the old code joined the array with
                // commas for <8 digits and emitted misaligned hex otherwise).
                if (Array.isArray(this.Value)) {
                    let result = 0n;
                    for (let idx = this.Value.length - 1; idx >= 0; idx--) {
                        result = (result << 30n) + BigInt(this.Value[idx]);
                    }
                    if (this.IsNegative) {
                        result = -result;
                    }
                ...
                // TYPE_INT64 (legacy, pre-3.4): 8-byte little-endian buffer.
                // Preserve the original hex rendering to avoid disturbing
                // existing snapshots for old fixtures.
                {
                    let result = "0x";
