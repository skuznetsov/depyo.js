...
def test9(arg_1=55, *varargs: int, y, **kwargs):
...
def test16(host=None, port=None, *, loop, limit, **kwds):
...
def div(a: dict(float, "the dividend", **(type, help)), b: dict(float, "the divisor (must be different than 0)", **(type, help))) -> dict(float, "the result of dividing a by b", **(type, help)):
    return a / b
...
class TestSignatureObject4:
    def test_signature_on_wkwonly(self):
        def test(x=55, *args, c: str, a: float, kwargs: str, **b: int) -> int:
            pass

class TestSignatureObject5:
    def test_signature_on_wkwonly(self):
        def test(x=55, *args: int, c, a: float, kwargs: str, **b: int) -> int:
            pass

class TestSignatureObject5:
    def test_signature_on_wkwonly(self):
        def test(x: int=55, *args: (int, str), c, a: float, kwargs: str, **b: int) -> int:
            pass
...
assert test1(1, 5, 6, "bar", **(foo)) == (1, 5, 6, {"foo": "bar"})
assert test2(2, 3, 4) == (2, 3, 4, 4, (), {})
assert test3(10, "bar", **(foo)) == 5.4
...
assert test13(1, 2, 3, "hi", **(name)) == ((1, 2, 3), hi), "*args, name"
